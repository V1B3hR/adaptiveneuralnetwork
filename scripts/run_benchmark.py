...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Run benchmarks for adaptive neural networks")

    # Dataset selection
    parser.add_argument(
        "--dataset", choices=["mnist"], default="mnist", help="Dataset to use for benchmarking"
    )
    ...
    # Testing options
    parser.add_argument(
        "--quick-test", action="store_true", help="Run quick test with subset of data"
    )

    parser.add_argument(
        "--subset-size", type=int, default=None, help="Use subset of training data (for testing)"
    )
...
def main():
    ...
    if args.dataset == "mnist":
        if args.quick_test:
            ...
            results = quick_mnist_test(
                num_epochs=min(args.epochs, 2), subset_size=args.subset_size or 1000
            )
        else:
            print("Running full MNIST benchmark...")
            results = run_mnist_benchmark(
                config=config, subset_size=args.subset_size, save_results=not args.no_save
            )
    ...
    if final_metrics:
        ...
        print(
            f"  Training Time:  {results.get('benchmark_info', {}).get('total_training_time', 'N/A'):.2f}s"
        )
