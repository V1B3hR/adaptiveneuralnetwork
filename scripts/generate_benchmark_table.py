...
class BenchmarkTableGenerator:
    ...
    def extract_metric_value(
        self, data: Dict[str, Any], metric_path: str, default: str = "TBD"
    ) -> str:
                ...
                if isinstance(main_data, dict):
                    ...
                    if "experiments" in main_data:
                        for exp in main_data["experiments"]:
                            if (
                                isinstance(exp, dict)
                                and config["dataset"].lower() in str(exp).lower()
                            ):
                                test_acc = self.extract_metric_value(
                                    exp, config["acc_path"], test_acc
                                )
                                active_nodes = self.extract_metric_value(
                                    exp, config["nodes_path"], active_nodes
                                )
                    ...
                    if "metrics" in main_data:
                        ...
                        active_nodes = self.extract_metric_value(
                            metrics, config["nodes_path"], active_nodes
                        )
        ...
        for config in robustness_configs:
            ...
            for source in result_sources:
                if isinstance(source, dict):
                    success_rate = self.extract_metric_value(
                        source, config["success_path"], success_rate
                    )
                    recovery_time = self.extract_metric_value(
                        source, config["recovery_path"], recovery_time
                    )
                    resilience_score = self.extract_metric_value(
                        source, config["resilience_path"], resilience_score
                    )
            ...
            for result_key, result_data in self.results.items():
                if isinstance(result_data, dict):
                    ...
                    if "performance" in result_data:
                        ...
                        training_time = self.extract_metric_value(
                            perf_data, "training_time", training_time
                        )
                        memory_usage = self.extract_metric_value(
                            perf_data, "memory_usage", memory_usage
                        )
                        energy_efficiency = self.extract_metric_value(
                            perf_data, "energy_efficiency", energy_efficiency
                        )
                    ...
                    if "timing" in result_data:
                        ...
                        training_time = self.extract_metric_value(
                            timing_data, "total_time", training_time
                        )
...
def main():
    ...
    parser.add_argument(
        "--readme-path", type=str, default="README.md", help="Path to README.md file to update"
    )
    parser.add_argument(
        "--update-readme",
        action="store_true",
    ...
    parser.add_argument(
        "--print-only", action="store_true", help="Only print tables to stdout, don't update files"
    )
