...
test_data = torch.randn(
    batch_size, config.sequence_length, 3, config.input_height, config.input_width
)
print(f"Test data shape: {test_data.shape}")
print()

# Compare different models
models_to_compare = {"ConvLSTM": "convlstm", "3D CNN": "conv3d", "Transformer": "transformer"}
...
for name, stats in results.items():
    params_str = (
        f"{stats['parameters'] / 1000:.0f}K"
        if stats["parameters"] < 1000000
        else f"{stats['parameters'] / 1000000:.1f}M"
    )
...
synthetic_video = create_synthetic_video_sequence(
    num_frames=config.sequence_length, height=config.input_height, width=config.input_width
)
...
for width, height, seq_len in input_configurations:
    ...
    print(
        f"{width:3d}x{height:3d} @ {seq_len:2d} frames -> {avg_latency:6.2f} ± {std_latency:5.2f}ms"
    )
...
export_config = VideoModelConfig(
    input_height=224, input_width=224, sequence_length=8, hidden_dim=128, num_classes=1000
)
...
print(
    f"  Input shape: (batch_size, {export_config.sequence_length}, 3, {export_config.input_height}, {export_config.input_width})"
)
