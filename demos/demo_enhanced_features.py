...
def create_demo_network(num_nodes=5):
    ...
    for i in range(num_nodes):
        ...
        node = AliveLoopNode(
            position=(x, y), velocity=(0, 0), initial_energy=np.random.uniform(10, 20), node_id=i
        )
...
def simulate_external_signals(network, mock_sources, intensity_factor=1.0):
    ...
    for source_type, data_generator in mock_sources.items():
        ...
        if source_type == "human":
            ...
            changes[StateVariable.ENERGY] = (
                (mock_data["energy_level"] - 0.5) * 2.0 * intensity_factor
            )
            ...
            if mock_data["air_quality"] > 100:
                changes[StateVariable.ANXIETY] = (
                    (mock_data["air_quality"] - 100) * 0.02 * intensity_factor
                )

            # Noise affects anxiety
            if mock_data["noise_level"] > 60:
                changes[StateVariable.ANXIETY] = (
                    changes.get(StateVariable.ANXIETY, 0)
...
def run_anxiety_overwhelm_scenario(network):
    ...
    logger.info(
        f"Node {victim_node.node_id} anxiety level: {victim_node.anxiety:.2f} (threshold: {victim_node.anxiety_threshold:.2f})"
    )
    ...
    for node in network.nodes:
        ...
        print(
            f"  Node {node.node_id}: Anxiety={node.anxiety:.2f}, Energy={node.energy:.2f}, "
    ...
    for node in network.nodes:
        if node.node_id == victim_node.node_id:
            print(
                f"  Node {node.node_id}: Anxiety={node.anxiety:.2f} (reduced), Energy={node.energy:.2f}"
            )
        else:
...
def demonstrate_time_series_tracking(network, num_steps=20):
    ...
    if network.enable_time_series:
        ...
        logger.info(
            f"Time series data collected: {stats['total_points']} points for {len(stats['unique_nodes'])} nodes"
        )
        ...
        for node in network.nodes:
            latest = network.time_series_tracker.get_latest_values(
                node.node_id, ["anxiety", "energy", "calm"]
            )
...
def demonstrate_security_features():
    ...
    secure_source = SignalSource(
        ...
        mappings=[
            ...
            SignalMapping(
                "stress_hormones", StateVariable.ANXIETY, "logarithmic", 2.0, 0.0, 0.0, 8.0
            ),
        ],
    ...
    print(
        f"Sources with private data: {len([s for s, info in privacy_report['sources'].items() if info['privacy_level'] in ['private', 'confidential']])}"
    )
    ...
    try:
        ...
        if network.enable_time_series:
            ...
            timeseries_file = network.time_series_tracker.export_data(
                query, format="json", output_path="demo_timeseries"
            )
            logger.info(f"Exported time series data to {timeseries_file}")

        # Export comprehensive network data
        network_file = network.export_network_data(format="json", output_path="demo_full_network")
...
def print_demo_summary(network, results):
    ...
    print("\nFeature Demonstrations:")
    print("  ✓ External signal absorption: Multi-modal signal integration")
    ...
    print("  ✓ Security features: Privacy controls and data protection")
    ...
    for node in network.nodes:
        ...
        print(
            f"  Node {node.node_id}: Anxiety={status['anxiety_level']:.2f}, "
...
def main():
    ...
    logger.info(
        f"Configuration: {args.nodes} nodes, {args.steps} steps, visualize={args.visualize}"
    )
