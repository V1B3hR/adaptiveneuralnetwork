...
def demo_anxiety_analysis():
    ...
    config = AdaptiveConfig(num_nodes=6, hidden_dim=10, input_dim=32, output_dim=3, device="cpu")

    model = AdaptiveModel(config)

    stress_conditions = {"high_loss_threshold": 2.0, "stress_duration": 5, "recovery_duration": 8}
...
def demo_enhanced_metrics():
    ...
    config = AdaptiveConfig(num_nodes=8, hidden_dim=12, device="cpu")
