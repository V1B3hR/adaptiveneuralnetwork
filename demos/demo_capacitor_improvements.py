...
def main():
    ...
    bounded_cap = CapacitorInSpace(
        position=[5.0, -8.0], capacity=15.0, bounds=bounds, expected_dims=2
    )
    ...
    print("-" * 40)
    fixed_cap = CapacitorInSpace(position=[2.0, 3.0, 4.0], capacity=30.0, fixed_position=True)
    ...
    threadsafe_cap = CapacitorInSpace(
        position=[0, 0], capacity=100.0, initial_energy=50.0, thread_safe=True
    )
    ...
    debug_cap = CapacitorInSpace(
        position=[0, 0], capacity=10.0, initial_energy=5.0, logger=logger, verbosity=logging.DEBUG
    )
