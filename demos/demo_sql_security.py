...
def demonstrate_performance_analysis():
    ...
    for node_id in range(1, 6):
        for i in range(100):
            tracker.record_node_state(
                node_id,
                {"energy": 10.0 + i * 0.1, "anxiety": 2.0 + i * 0.05, "trust": 0.5 + i * 0.001},
            )
...
def demonstrate_secure_integration():
    ...
    for i, node in enumerate(nodes):
        ...
        print(
            f"✓ Node {node.node_id} tracked with {len(tracker.get_latest_values(node.node_id))} variables"
        )
