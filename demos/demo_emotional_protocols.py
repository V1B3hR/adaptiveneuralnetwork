...
def create_emotional_network():
    ...
    neutral_node = AliveLoopNode(
        position=(1.5, -2), velocity=(0, 0.1), initial_energy=12.0, node_id=4
    )
    ...
    capacitors = [CapacitorInSpace(position=(1.5, 0), capacity=15.0, initial_energy=10.0)]
...
def demonstrate_joy_sharing(nodes):
    ...
    for node in nodes:
        print(
            f"  Node {node.node_id}: valence={node.emotional_state['valence']:.2f}, "
    ...
    recipients = joyful_node.send_joy_share_signal(
        nearby_nodes=other_nodes, joy_content=joy_content
    )
    ...
    for node in nodes[1:]:
        if node in recipients:
            print(
                f"  ✨ Node {node.node_id}: valence improved to {node.emotional_state['valence']:.2f}, "
...
def demonstrate_grief_support(nodes):
    ...
    print(
        f"     Current state: valence={grieving_node.emotional_state['valence']:.2f}, "
    ...
    supporters = grieving_node.send_grief_support_request(
        nearby_nodes=potential_supporters, grief_details=grief_details
    )
    ...
    print("\nEmotional Recovery:")
    print(
        f"  🌱 Node {grieving_node.node_id}: valence improved to {grieving_node.emotional_state['valence']:.2f}, "
    ...
    if support_memories:
        print(
            f"   🧠 Support received memory created (emotional valence: {support_memories[0].emotional_valence:.2f})"
        )
...
def demonstrate_celebration_and_comfort(nodes):
    ...
    if response:
        ...
        print(
            f"   📈 Emotional boost: valence {initial_valence:.2f} → {neutral_node.emotional_state['valence']:.2f}"
        )
...
def demonstrate_trust_evolution(nodes):
    ...
    for signal_type, source, target in interactions:
        ...
        print(
            f"  {signal_type}: Node {source.node_id} → Node {target.node_id}, "
...
def run_network_simulation(nodes, capacitors):
            ...
            for node in nodes:
                if node.emotional_state["valence"] > 0.5 and node.energy > 5:
                    nearby = [
                        ...
                        and np.linalg.norm(np.array(node.position) - np.array(n.position)) < 3.0
                    ]
                    if nearby:
    ...
    for node in nodes:
        ...
        print(
            f"  Node {node.node_id}: valence={valence:.2f}, anxiety={anxiety:.2f}, "
