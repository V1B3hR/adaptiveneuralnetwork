...
from adaptiveneuralnetwork.api.backend_factory import list_available_backends, get_backend_info
...
from adaptiveneuralnetwork.core.neuromorphic import create_neuromorphic_model, NeuromorphicPlatform
...
def demo_backend_selection():
    ...
    if backends["jax"]:
        try:
            ...
            print(f"  ✓ JAX model: Created successfully")
        except Exception as e:
            print(f"  ✗ JAX model failed: {e}")
    else:
        print(f"  - JAX model: Skipped (JAX not available)")
    ...
    try:
        ...
        for corruption in test_corruptions:
            ...
            for severity in test_severities:
                ...
                print(f"    Severity {severity}: {corrupted_acc:.3f} accuracy ({corruption_error:.3f} error)")
        ...
        if relative_robustness > 0.8:
            print(f"     Rating: 🟢 Excellent robustness")
        elif relative_robustness > 0.6:
            print(f"     Rating: 🟡 Good robustness")
        else:
...
def demo_multimodal_learning(quick_mode=False):
    ...
    try:
        ...
        if results['final_test_accuracy'] > 0.6:
            print(f"     Status: 🟢 Successfully learning multi-modal patterns")
        elif results['final_test_accuracy'] > 0.4:
            print(f"     Status: 🟡 Moderate multi-modal learning")
        else:
            print(f"     Status: 🔴 Struggling with cross-modal fusion")
...
def demo_neuromorphic_processing(quick_mode=False):
    ...
    try:
        ...
        if spikiness > 0.1:
            print(f"     • High spike activity - good for neuromorphic hardware")
        else:
            print(f"     • Low spike activity - may need tuning for hardware")
            
        print(f"     • Event-driven: Processes only when input changes")
        print(f"     • Low power: Computation only on spike events")
        print(f"     • Asynchronous: No global clock synchronization")
...
def main():
    ...
    parser.add_argument(
        "--quick", 
        action="store_true",
        help="Run in quick mode (smaller models, less data)"
    )
    parser.add_argument(
        "--demo",
