...
from core.spatial_utils import distance
...
def demo_2d_vs_3d_comparison():
    ...
    for dimension_name, spatial_dims in [("2d", 2), ("3d", 3)]:
        ...
        for i in range(3):
            ...
            cap = CapacitorInSpace(
                position=position, capacity=5.0, initial_energy=2.0, expected_dims=spatial_dims
            )
            capacitors.append(cap)

        print(
            f"Created {len(nodes)} nodes and {len(capacitors)} capacitors in {spatial_dims}D space"
        )
    ...
    print("\n--- Comparison Summary ---")
    print(
        f"2D Network: {results['2d']['avg_energy']:.2f} energy, {results['2d']['total_interactions']} interactions"
    )
    print(
        f"3D Network: {results['3d']['avg_energy']:.2f} energy, {results['3d']['total_interactions']} interactions"
    )
    ...
    if results["2d"]["interaction_density"] > 0 and results["3d"]["interaction_density"] > 0:
        density_diff = 100 * (
            1 - results["3d"]["interaction_density"] / results["2d"]["interaction_density"]
        )
        ...
        print(
            f"Interaction densities: 2D={results['2d']['interaction_density']:.3f}, 3D={results['3d']['interaction_density']:.3f}"
        )

    print(
        "Note: In 3D, nodes have more space to move, typically resulting in fewer chance encounters"
    )
...
def demo_dimensional_scaling():
    ...
    for dim in dimensions_to_test:
        ...
        node = AliveLoopNode(
            position=[0] * dim, velocity=[0.1] * dim, initial_energy=10.0, spatial_dims=dim
        )
...
def demo_3d_configuration_switching():
    ...
    for i in range(3):
        ...
        node = AliveLoopNode(
            position=position, velocity=velocity, spatial_dims=spatial_dims, node_id=i
        )
        nodes.append(node)
