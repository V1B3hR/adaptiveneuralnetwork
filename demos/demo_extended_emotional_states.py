...
def create_emotional_demo_nodes():
    ...
    joyful_node = AliveLoopNode(position=(0, 0), velocity=(0.1, 0), initial_energy=18.0, node_id=1)
    joyful_node.emotional_state["valence"] = 0.8
    joyful_node.calm = 4.0
    joyful_node.joy = 3.5
    print(
        f"   😊 Joyful Node (ID: {joyful_node.node_id}): joy={joyful_node.joy}, calm={joyful_node.calm}"
    )

    # Grieving node with high grief and sadness
    grieving_node = AliveLoopNode(
        position=(3, 0), velocity=(-0.05, 0), initial_energy=8.0, node_id=2
    )
    ...
    print(
        f"   😢 Grieving Node (ID: {grieving_node.node_id}): grief={grieving_node.grief}, sadness={grieving_node.sadness}, anxiety={grieving_node.anxiety}"
    )

    # Supportive node with balanced emotional state
    supportive_node = AliveLoopNode(
        position=(1.5, 2), velocity=(0, -0.1), initial_energy=15.0, node_id=3
    )
    ...
    print(
        f"   🤝 Supportive Node (ID: {supportive_node.node_id}): calm={supportive_node.calm}, joy={supportive_node.joy}"
    )
...
def demonstrate_emotional_history_tracking(nodes):
    ...
    print(
        f"   Initial state: joy={node.joy:.2f}, grief={node.grief:.2f}, sadness={node.sadness:.2f}"
    )
    ...
    for step in range(5):
        ...
        print(
            f"   Step {step + 1}: joy={node.joy:.2f}, grief={node.grief:.2f}, sadness={node.sadness:.2f}"
        )

    print(
        f"   History lengths: joy={len(node.joy_history)}, grief={len(node.grief_history)}, sadness={len(node.sadness_history)}"
    )
...
def demonstrate_emotional_prediction(node):
    ...
    for state in ["joy", "grief", "sadness", "calm", "anxiety"]:
        ...
        print(
            f"   {state.capitalize()}: current={current:.2f}, "
...
def demonstrate_enhanced_emotional_interactions(nodes):
    ...
    joy_signal = SocialSignal(
        content=joy_content, signal_type="joy_share", urgency=0.4, source_id=joyful_node.node_id
    )

    print(
        f"   Grieving node before joy sharing: joy={grieving_node.joy:.2f}, "
    ...
    print(
        f"   Grieving node after joy sharing: joy={grieving_node.joy:.2f}, "
        f"sadness={grieving_node.sadness:.2f}, grief={grieving_node.grief:.2f}"
    )
    ...
    for step in range(steps):
        ...
        for node in nodes:
            ...
            if step % 3 == 0:
                ...
                if assessment["intervention_needed"]:
                    print(
                        f"     Node {node.node_id} needs intervention: {assessment['intervention_type']}"
                    )
        ...
        if step % 5 == 4:  # Every 5 steps
            ...
            for node in nodes:
                print(
                    f"     Node {node.node_id}: joy={node.joy:.1f}, grief={node.grief:.1f}, "
