...
def demonstrate_normal_trust_evolution():
    ...
    for signal_type, description in interactions:
        ...
        print(f"📈 {description}")
        print(
            f"   Trust: {trust_before:.3f} → {trust_after:.3f} (Δ{trust_after - trust_before:+.3f})"
        )
        ...
        if bob.node_id in alice.trust_network_system.suspicion_alerts:
            ...
            print(
                f"   ⚠️  ALERT: {alert['status']} - {alice.trust_network_system._generate_suspicion_reason(bob.node_id, signal_type)}"
            )
...
def demonstrate_manipulation_detection():
    ...
    print("Manipulator attempting love bombing attack on victim...")
    ...
    for i, (signal_type, description) in enumerate(love_bomb_signals):
        ...
        if manipulator.node_id in victim.trust_network_system.suspicion_alerts:
            ...
            print(
                f"   Reason: {victim.trust_network_system._generate_suspicion_reason(manipulator.node_id, signal_type)}"
            )
            break
