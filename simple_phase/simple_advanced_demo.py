...
def test_all_features():
    ...
    try:
        ...
        assert len(available_models) >= 4, f"Expected at least 4 neuron models, got {len(available_models)}"
    ...
    try:
        ...
        import sys, os
        sys.path.append(os.path.join(os.path.dirname(__file__), 'adaptiveneuralnetwork', 'applications'))
        
        # Read the file content and extract the class manually to avoid dependencies
        with open('adaptiveneuralnetwork/applications/continual_learning.py', 'r') as f:
            content = f.read()
        
        # Check that NonStationaryDataHandler exists in the file
        assert 'class NonStationaryDataHandler:' in content, "NonStationaryDataHandler class not found"
        assert 'detect_distribution_shift' in content, "Distribution shift detection method not found"
