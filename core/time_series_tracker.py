...
class TimeSeriesTracker:
    ...
    def __init__(
        ...
        self._memory_store: Dict[int, Dict[str, deque]] = defaultdict(
    ...
    def record_node_state(
        self, node_id: int, state_data: Dict[str, Any], timestamp: Optional[float] = None
    ):
        """
        ...
        with self._lock:
            for variable_name, value in state_data.items():
                ...
                if variable_name not in self.ALLOWED_VARIABLES:
                    logger.warning(
                        f"Variable '{variable_name}' not in standard allowlist. Consider adding to ALLOWED_VARIABLES."
                    )
                ...
                if isinstance(value, (int, float)):
                    ...
                    if (
                        self._stats["first_timestamp"] is None
                        or timestamp < self._stats["first_timestamp"]
                    ):
                        self._stats["first_timestamp"] = timestamp
                    if (
                        self._stats["last_timestamp"] is None
                        or timestamp > self._stats["last_timestamp"]
                    ):
    ...
    def query(self, query: TimeSeriesQuery) -> List[TimeSeriesDataPoint]:
        ...
        with self._lock:
            ...
            if (
                len(results) < (query.max_points or 1000)
                and self.persist_to_disk
                and (query.start_time is not None or query.end_time is not None)
            ):
        ...
        if query.variables is not None:
            for var_name in query.variables:
                ...
                if var_name not in self.ALLOWED_VARIABLES:
                    raise ValueError(
                        f"Variable '{var_name}' not in allowed list: {sorted(self.ALLOWED_VARIABLES)}"
                    )
    ...
    def _query_database(self, query: TimeSeriesQuery) -> List[TimeSeriesDataPoint]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Build SQL query
                sql_parts = [
                    "SELECT timestamp, node_id, variable_name, value, metadata FROM time_series WHERE 1=1"
                ]
                params = []
    ...
    def get_latest_values(
        self, node_id: int, variables: Optional[List[str]] = None
    ) -> Dict[str, float]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if stats["first_timestamp"]:
            stats["first_timestamp_formatted"] = datetime.fromtimestamp(
                stats["first_timestamp"]
            ).isoformat()
        if stats["last_timestamp"]:
            stats["last_timestamp_formatted"] = datetime.fromtimestamp(
    ...
    def visualize_node_variables(
        ...
        for i, variable in enumerate(variables):
            ...
            if data["times"]:
                ...
                if len(data["values"]) > 10:
                    ...
                    ax.plot(
                        data["times"],
                        p([t.timestamp() for t in data["times"]]),
                        "r--",
                        alpha=0.7,
                        linewidth=2,
        ...
        axes[-1].xaxis.set_major_locator(
            mdates.HourLocator(interval=max(1, int(time_range_hours / 12)))
        )
    ...
    def export_data(
        self, query: TimeSeriesQuery, format: str = "json", output_path: str = "timeseries_export"
    ) -> str:
        """
        ...
        with self._lock:
            for node_id in list(self._memory_store.keys()):
                for variable_name in list(self._memory_store[node_id].keys()):
                    ...
                    new_points = deque(
                        [p for p in points if p.timestamp >= cutoff_time], maxlen=points.maxlen
                    )
