...
def rand_vector(
    dim: int, ranges: Union[Tuple[float, float], Sequence[Tuple[float, float]]]
) -> np.ndarray:
...
def expand_bounds_to_dimensions(
    bounds: Union[Tuple[float, float], Sequence[Tuple[float, float]]], dim: int
...
def validate_position_in_bounds(
    position: np.ndarray, bounds: Sequence[Tuple[float, float]]
) -> None:
    ...
    if len(bounds) != position.shape[0]:
        raise ValueError(
            f"Bounds count {len(bounds)} doesn't match position dimensions {position.shape[0]}"
        )

    for i, ((min_val, max_val), coord) in enumerate(zip(bounds, position)):
        if not (min_val <= coord <= max_val):
            raise ValueError(
                f"Position component {i}={coord} outside bounds [{min_val}, {max_val}]"
            )


def create_random_positions(
    count: int, dim: int, bounds: Union[Tuple[float, float], Sequence[Tuple[float, float]]]
) -> np.ndarray:
