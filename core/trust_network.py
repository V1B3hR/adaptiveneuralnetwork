...
class TrustNetwork:
    ...
    def update_trust(self, target, signal_type, context=None):
        ...
        trust_delta = max(
            -self.TRUST_VOLATILITY_LIMIT, min(self.TRUST_VOLATILITY_LIMIT, trust_delta)
        )
    ...
    def _detect_manipulation_pattern(self, node_id):
        ...
        positive_count = sum(1 for h in recent[-5:] if h["signal_type"] in positive_signals)
        if positive_count >= 4:
            return True
        ...
        if len(recent) >= 6:
            ...
            if len(pattern) >= 6:
                if pattern == [True, False, True, False, True, False] or pattern == [
                    False,
                    True,
                    False,
                    True,
    ...
    def _broadcast_to_trusted_neighbors(self, verification_request):
        ...
        for neighbor in trusted_neighbors[:5]:  # Ask up to 5 trusted neighbors
            ...
            print(
                f"Requesting trust verification from {neighbor} about {verification_request['subject']}"
            )
    ...
    def get_trust_summary(self):
        ...
        summary = {
            ...
            "active_alerts": sum(
                1 for a in self.suspicion_alerts.values() if a["status"] == "pending_verification"
            ),
            "paranoia_warning": paranoid_check
            > len(trust_values) * 0.3,  # If >30% are below paranoia threshold
        }
