...
class AliveLoopNode:
    ...
    def update_size(self):
        self.radius = min(2.0, 0.1 + 0.05 * self.energy)
    ...
    def seek_capacitors(self, capacitors):
        ...
        if best_capacitor is not None:
            ...
            if np.linalg.norm(direction) > 0:
                self.velocity = 0.1 * direction / np.linalg.norm(direction)
    ...
    def interact_with_capacitor(self, capacitor, threshold=0.5):
        ...
        if effective_distance < threshold:
            ...
            reward_gain /= (1.0 + self.reward / 10.0)
            if "inspired" in self.phase_mix:
                reward_gain *= 2
            if "interactive" in self.phase_mix:
                reward_gain *= 1.3
            ...
            if self.reward > 10:
                capacitor.energy += 0.5 * (self.reward - 10)
                self.reward = 10
