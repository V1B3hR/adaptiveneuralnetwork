...
def log_ethics_event(action: str, audit_result: Dict[str, Any]) -> None:
    ...
    if audit_result["compliant"]:
        ...
        ethics_logger.warning(
            f"VIOLATION: Action '{action}' failed ethics audit: {audit_result['violations']}"
        )
...
def enforce_ethics_compliance(decision_log: Dict[str, Any]) -> None:
    ...
    if not audit_result["compliant"]:
        raise RuntimeError(
            f"Ethics violation: {audit_result['violations']} in action '{decision_log.get('action')}'"
        )
