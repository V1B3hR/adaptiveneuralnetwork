...
from adaptiveneuralnetwork.data.kaggle_datasets import load_pos_tagging_dataset, get_pos_dataset_statistics
from adaptiveneuralnetwork.models.pos_tagger import (
    POSTagger, POSTaggerConfig, compute_sequence_lengths, 
    compute_token_accuracy, masked_cross_entropy_loss
)
...
def compute_metrics(predictions: List[List[str]], labels: List[List[str]], tag_vocab: Dict[str, int]) -> Dict[str, Any]:
    """
    ...
    if HAS_SEQEVAL:
        ...
        metrics.update({
            'macro_f1': macro_f1,
            'micro_f1': micro_f1,
            'per_tag_f1': {tag: report.get(tag, {}).get('f1-score', 0.0) for tag in tag_vocab.keys() if tag != '<PAD>'}
        })
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train POS Tagger with Adaptive Heuristics"
    )
    ...
    parser.add_argument("--filter-punctuation", action="store_true", help="Filter punctuation tokens")
    ...
    parser.add_argument("--min-improve", type=float, default=0.001, help="Minimum improvement threshold")
    
    # Model arguments
    parser.add_argument("--model", choices=["bilstm", "transformer"], default="bilstm", help="Model type")
    ...
    parser.add_argument("--output-dir", type=str, default="./pos_tagging_output", help="Output directory")
    ...
    if args.synthetic or not args.data_path:
        ...
        datasets = {
            'train': POSDataset(synthetic_sentences[:train_size], synthetic_tags[:train_size], vocab, tag_vocab),
            'val': POSDataset(synthetic_sentences[train_size:train_size+val_size], synthetic_tags[train_size:train_size+val_size], vocab, tag_vocab),
            'test': POSDataset(synthetic_sentences[train_size+val_size:], synthetic_tags[train_size+val_size:], vocab, tag_vocab)
        }
        ...
        stats = {
            ...
            'avg_sentence_length': sum(len(s) for s in synthetic_sentences) / len(synthetic_sentences)
        }
    ...
    for epoch in range(heuristics['epochs']):
        ...
        train_metrics = train_epoch(
            ...
            heuristics['gradient_accumulation_steps'], progress_bar=not args.verbose
        )
        
        # Validate
        val_metrics = evaluate_model(
            model, data_loaders['val'], device, tag_vocab, idx_to_tag
        )
        ...
        # Log progress
        logger.info(f"Epoch {epoch+1}/{heuristics['epochs']} - "
    ...
    if 'test' in data_loaders:
        ...
        test_metrics = evaluate_model(
            model, data_loaders['test'], device, tag_vocab, idx_to_tag
        )
