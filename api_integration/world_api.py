...
class EnvironmentalSignalManager:
    ...
    def _setup_default_sources(self):
        ...
        weather_source = SignalSource(
            ...
            mappings=[
                ...
                SignalMapping("wind_speed", StateVariable.ENERGY, "logarithmic", 2.0, 0.0, 0.0, 10.0),
        ...
        news_source = SignalSource(
            ...
            mappings=[
                SignalMapping("sentiment_score", StateVariable.EMOTIONAL_VALENCE, "linear", 2.0, -1.0, -1.0, 1.0),
                SignalMapping("urgency_score", StateVariable.ANXIETY, "sigmoid", 3.0, 0.0, 0.0, 6.0),
                SignalMapping("relevance_score", StateVariable.ATTENTION_FOCUS, "linear", 1.0, 0.0, 0.0, 2.0),
        ...
        economic_source = SignalSource(
            ...
            mappings=[
                SignalMapping("volatility", StateVariable.ANXIETY, "logarithmic", 2.0, 0.0, 0.0, 4.0),
                ...
                SignalMapping("confidence_index", StateVariable.TRUST, "linear", 0.01, 0.0, 0.0, 1.0)
            ],
            privacy_level="public",
    ...
    def add_sensor_source(self, name: str, api_url: str, mappings: list, 
                         credentials: Optional[ApiCredentials] = None) -> None:
    ...
    def fetch_environmental_state_changes(self, source_name: str = None, 
                                        location_params: Optional[Dict] = None) -> Dict[StateVariable, float]:
        """
        ...
        if source_name:
            ...
            for source_name, signals in all_signals.items():
                ...
                if source.signal_type in [SignalType.ENVIRONMENTAL, SignalType.SENSOR, SignalType.ECONOMIC]:
            ...
            if source.signal_type in [SignalType.ENVIRONMENTAL, SignalType.SENSOR, SignalType.ECONOMIC]:
                ...
                if source.cached_data:
                    for mapping in source.mappings:
                        if (mapping.source_field in source.cached_data and 
                            mapping.target_variable == StateVariable.ANXIETY):
