...
def evaluate_model(
    model: POSTagger, dataset: POSDataset, device: torch.device, batch_size: int = 32
) -> Dict[str, Any]:
    """
...
def print_evaluation_report(results: Dict[str, Any], tag_vocab: Dict[str, int]) -> None:
    ...
    if "per_tag_metrics" in metrics:
        ...
        for tag, tag_metrics in sorted(metrics["per_tag_metrics"].items()):
            if tag != "<PAD>":
                print(
                    f"{tag:<10} {tag_metrics['precision']:<10.3f} "
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate POS Tagging Model")
