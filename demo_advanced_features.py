...
from adaptiveneuralnetwork.core.precision_phases import MixedPrecisionPhaseManager
...
def demo_formal_intelligence_evaluation():
    ...
    results = benchmark.run_comprehensive_benchmark(
        include_comparisons=True, include_robustness=True, formal_evaluation=True
    )
    ...
    try:
        ...
        print(
            f"   95% Confidence Interval: {formal_results['overall_statistics']['confidence_interval']}"
        )
...
def demo_probabilistic_phase_scheduling():
    ...
    scheduler = PhaseScheduler(
        num_nodes=10, stochastic_policy=True, policy_temperature=1.2, exploration_rate=0.15
    )

    print("   ✓ Scheduler created with stochastic policy")
    ...
    print("   Policy adjusted for poor performance:")
...
def demo_mixed_precision_phases():
    ...
    print("   ✓ Manager created with dynamic precision")
    ...
    for phase in [Phase.ACTIVE, Phase.SLEEP, Phase.INSPIRED]:
        output, metrics = precision_manager.compute_with_phase_precision(
            simple_computation, test_input, phase, complexity_score=0.8
        )
...
def demo_continual_learning_nonstationary():
    ...
    for batch_idx in range(20):
        ...
        if batch_idx % 5 == 0:
            print(
                f"   Batch {batch_idx}: shift={shift_detected}, adaptation={adaptation_info['adaptation_strategy']}"
            )
    ...
    if adaptation_samples is not None:
        ...
        print("   No adaptation samples available yet")
...
def create_integration_demo():
    ...
    scheduler = PhaseScheduler(
        num_nodes=8, stochastic_policy=True, policy_temperature=1.0, exploration_rate=0.1
    )

    # Precision manager
    precision_manager = MixedPrecisionPhaseManager(dynamic_precision=True, enable_amp=False)
