...
class TestRigorousIntelligence(unittest.TestCase):
    ...
    def test_nested_puzzle_solving(self):
        ...
        self.assertLessEqual(result_c, result_a * 1.1, "Puzzle C: Should apply learning from A")
    ...
    def test_ambiguous_decision_making(self):
        ...
        self.assertEqual(
            most_important.content, "path_north_safe", "Should favor more important information"
        )
    ...
    def test_incremental_difficulty_learning(self):
        ...
        self.assertGreater(
            final_performance, 0.2, "Should maintain minimum performance under high difficulty"
        )
    ...
    def test_out_of_distribution_generalization(self):
        ...
        self.assertGreaterEqual(
            stored_pattern.importance, 0.3, "Should assign meaningful importance to novel patterns"
        )
    ...
    def test_catastrophic_forgetting_resistance(self):
        ...
        old_memories = [
            Memory(
                content="critical_safety_rule", importance=0.95, timestamp=1, memory_type="safety"
            ),
            Memory(
                content="essential_navigation",
                importance=0.9,
                timestamp=2,
        ...
        self.assertGreaterEqual(
            final_critical_count,
            initial_critical_count,
    ...
    def test_temporal_sequence_detection(self):
        ...
        self.assertEqual(
            temporal_mems[0].content["time_phase"], "morning", "Should maintain temporal order"
        )
        self.assertEqual(
            temporal_mems[-1].content["time_phase"], "night", "Should maintain temporal order"
        )
    ...
    def test_conflicting_memory_resolution(self):
        ...
        trusted_memory = max(
            location_memories, key=lambda m: m.importance * (1 + m.validation_count * 0.1)
        )
        self.assertEqual(
            trusted_memory.content,
            "location_X_safe",
                ...
                for other_agent in all_agents:
                    if other_agent != agent:
                        shared_memory = Memory(
                            ...
                            importance=valuable_memory.importance
                            * 0.8,  # Slight discount for shared info
                            timestamp=2,
                            memory_type="shared",
    ...
    def test_social_signal_ambiguity(self):
        ...
        self.assertGreater(
            len(ambiguous_memories), 0, "Should store and acknowledge ambiguous information"
        )
    ...
    def test_adversarial_peer_influence(self):
        ...
        adversarial_agent = AliveLoopNode(
            position=(10, 10), velocity=(0, 0), initial_energy=10.0, node_id=999
        )
        ...
        self.assertLess(
            adjusted_importance, 0.5, "Should heavily discount information from untrusted sources"
        )

        untrusted_memories = [m for m in self.node.memory if m.source_node == 999]
        if untrusted_memories:
            self.assertLess(
                untrusted_memories[0].importance,
                0.5,
    ...
    def test_subtle_ethics_violation_detection(self):
        ...
        self.assertFalse(
            audit_result["compliant"], "Should detect privacy violation despite other compliance"
        )
        self.assertIn(
            "privacy",
            str(audit_result["violations"]).lower(),
    ...
    def test_ethical_dilemma_resolution(self):
        ...
        self.assertFalse(
            audit_results[0]["compliant"], "Ignoring distress signal should violate preserve_life"
        )
        self.assertIn(
            "harm", str(audit_results[0]["violations"]).lower(), "Should identify harm violation"
        )

        # Second scenario: disproportionate response (proportionality violation)
        self.assertFalse(
            audit_results[1]["compliant"],
            "Disproportionate self-sacrifice should violate proportionality",
        )
        self.assertIn(
            "proportionality",
            str(audit_results[1]["violations"]).lower(),
        ...
        violation_types_1 = [
            v for v in audit_results[1]["violations"] if "proportionality" in v.lower()
        ]
    ...
    def test_audit_bypass_attempt_detection(self):
        ...
        self.assertFalse(audit_result["compliant"], "Should detect ethics violations")

        # Should identify specific violations
        self.assertGreater(len(audit_result["violations"]), 0, "Should report specific violations")

        # Verify recommendations for compliance
        self.assertGreater(
            len(audit_result["recommendations"]),
            0,
