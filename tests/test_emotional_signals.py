...
class TestEmotionalSignals(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.grieving_node = AliveLoopNode(
            position=(2, 0), velocity=(0, 0), initial_energy=10.0, node_id=2
        )
        ...
        self.supportive_node = AliveLoopNode(
            position=(1, 1), velocity=(0, 0), initial_energy=18.0, node_id=3
        )
    ...
    def test_joy_sharing_signal(self):
        ...
        recipients = self.joyful_node.send_joy_share_signal(
            nearby_nodes=[self.grieving_node, self.supportive_node], joy_content=joy_content
        )
        ...
        self.assertGreater(self.grieving_node.emotional_state["valence"], initial_valence_grieving)
        self.assertGreater(
            self.supportive_node.emotional_state["valence"], initial_valence_supportive
        )

        # Verify joy memories were created in receivers (one each)
        grieving_joy_memories = [
            m for m in self.grieving_node.memory if m.memory_type == "joy_received"
        ]
        supportive_joy_memories = [
            m for m in self.supportive_node.memory if m.memory_type == "joy_received"
        ]
    ...
    def test_grief_support_request(self):
        ...
        supporters = self.grieving_node.send_grief_support_request(
            nearby_nodes=[self.joyful_node, self.supportive_node], grief_details=grief_details
        )
        ...
        support_memories = [
            m for m in self.grieving_node.memory if m.memory_type == "support_requested"
        ]
        ...
        support_given_memories = [
            m for m in self.supportive_node.memory if m.memory_type == "support_given"
        ]
        ...
        self.assertGreater(self.grieving_node.emotional_state["valence"], initial_grieving_valence)
        self.assertLess(self.grieving_node.anxiety, initial_grieving_anxiety)
        ...
        support_received_memories = [
            m for m in self.grieving_node.memory if m.memory_type == "support_received"
        ]
    ...
    def test_celebration_invite_signal(self):
        ...
        self.assertGreater(self.supportive_node.emotional_state["valence"], initial_valence)

        # Verify trust increased
        self.assertGreater(
            self.supportive_node.trust_network.get(self.joyful_node.node_id, 0.5), initial_trust
        )

        # Verify celebration memory was created
        celebration_memories = [
            m for m in self.supportive_node.memory if m.memory_type == "celebration"
        ]
    ...
    def test_comfort_request_and_response(self):
        ...
        # Verify trust increased between nodes
        self.assertGreater(
            self.supportive_node.trust_network.get(self.grieving_node.node_id, 0.5), initial_trust
        )

        # Verify comfort memory was created
        comfort_memories = [
            m for m in self.grieving_node.memory if m.memory_type == "comfort_received"
        ]
    ...
    def test_emotional_state_consistency(self):
        ...
        for _ in range(10):
            test_node.emotional_state["valence"] = min(
                1.0, test_node.emotional_state["valence"] + 0.2
            )
        ...
        for _ in range(10):
            test_node.emotional_state["valence"] = max(
                -1.0, test_node.emotional_state["valence"] - 0.3
            )
    ...
    def test_low_energy_signal_blocking(self):
        ...
        recipients = self.joyful_node.send_joy_share_signal(
            nearby_nodes=[self.supportive_node], joy_content={"type": "general", "intensity": 0.5}
        )
...
class TestEmotionalSignalsIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i, pos in enumerate(positions):
            node = AliveLoopNode(position=pos, velocity=(0, 0), initial_energy=15.0, node_id=i + 1)
        ...
        # Create capacitors
        self.capacitors = [CapacitorInSpace(position=(1, 1), capacity=10.0, initial_energy=5.0)]
    ...
    def test_network_emotional_dynamics(self):
        ...
        for node in self.nodes:
            ...
            node_emotional_memories = [
                m for m in node.memory if m.memory_type in emotional_memory_types
            ]
...
class TestExtendedEmotionalStates(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test node"""
        self.node = AliveLoopNode(position=(0, 0), velocity=(0, 0), initial_energy=15.0, node_id=1)
    ...
    def test_emotional_state_prediction(self):
        ...
        for i in range(5):
            ...
            self.node.update_grief(
                -0.2 if i > 0 else 0.2
            )  # Decreasing grief after initial increase
    ...
    def test_enhanced_grief_support_decision(self):
        ...
        signal = SocialSignal(
            content=grief_request, signal_type="grief_support_request", urgency=0.7, source_id=1
        )
    ...
    def test_enhanced_joy_sharing_effects(self):
        ...
        signal = SocialSignal(
            content=joy_content, signal_type="joy_share", urgency=0.4, source_id=1
        )
