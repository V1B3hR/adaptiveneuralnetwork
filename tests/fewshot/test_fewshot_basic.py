...
def test_fewshot_learning_basic():
    ...
    def create_fewshot_data(k_shot: int, seed: int):
        ...
        class_0_data = torch.randn(k_shot, 32) + torch.tensor(
            [2.0] + [0.0] * 31
        )  # High first feature
        class_1_data = torch.randn(k_shot, 32) + torch.tensor(
            [0.0, 2.0] + [0.0] * 30
        )  # High second feature
        class_2_data = torch.randn(k_shot, 32) + torch.tensor(
            [0.0, 0.0, 2.0] + [0.0] * 29
        )  # High third feature
    ...
    for k_shot in [2, 5]:  # 2-shot and 5-shot
        ...
        train_loader = DataLoader(
            TensorDataset(X_train, y_train), batch_size=3 * k_shot, shuffle=True
        )
        ...
        assert (
            accuracy > chance_level + min_improvement
