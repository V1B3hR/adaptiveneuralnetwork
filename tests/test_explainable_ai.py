...
from core.explainable_ai import (
    ExplainableDecisionLogger, DecisionType, ReasoningStep,
    DecisionLog, ReasoningTrace, EthicalFactor, TrustCalculation
)
...
class TestExplainableDecisionLogger(unittest.TestCase):
    ...
    def test_finalize_decision(self):
        ...
        self.logger.add_reasoning_step(
            decision_id, ReasoningStep.EVALUATION, "Evaluating consensus options",
            {"options": ["A", "B", "C"]}, {"method": "voting"}, {"winner": "B"}, 0.8
        )
        
        # Add ethical factor with high compliance
        self.logger.add_ethical_factor(
            decision_id, "Respect Autonomy", "Universal Law #7", 0.8, "Good", 0.85, "All agents consulted"
        )
    ...
    def test_invalid_decision_id_handling(self):
        ...
        with self.assertRaises(ValueError):
            self.logger.add_ethical_factor(
                invalid_id, "test", "test", 1.0, "test", 0.5, "test"
            )
    ...
    def _create_complete_decision(self, agent_id: int = 1) -> str:
        ...
        # Add reasoning steps
        self.logger.add_reasoning_step(
            decision_id, ReasoningStep.OBSERVATION, "Observing situation",
            {"observations": ["A", "B"]}, {"method": "direct"}, {"summary": "observed"}, 0.9
        )
        
        self.logger.add_reasoning_step(
            decision_id, ReasoningStep.ANALYSIS, "Analyzing options",
            {"options": ["X", "Y"]}, {"method": "comparison"}, {"best": "X"}, 0.8
        )
