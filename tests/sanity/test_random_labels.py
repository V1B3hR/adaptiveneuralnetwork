...
def test_random_labels_poor_performance(make_loader):
    ...
    assert (
        accuracy < 0.8
    ), f"Accuracy {accuracy:.3f} too high for random labels - possible data leakage"
    ...
    assert (
        abs(accuracy - chance_level) < margin
...
def test_random_vs_structured_labels_difference(make_loader):
    ...
    y_structured = (
        (X_structured[:, :3].sum(dim=1) > 0).long()
        + (X_structured[:, 3:6].sum(dim=1) > 0).long() * 2
    ) % 5

    X_val_structured = torch.randn(50, 32)
    y_val_structured = (
        (X_val_structured[:, :3].sum(dim=1) > 0).long()
        + (X_val_structured[:, 3:6].sum(dim=1) > 0).long() * 2
    ) % 5
