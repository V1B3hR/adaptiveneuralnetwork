...
def test_permutation_degrades_performance(make_loader):
    ...
    train_loader_permuted = DataLoader(
        TensorDataset(X_train_permuted, y_train), 
        batch_size=32, 
        shuffle=True
    )
    val_loader_permuted = DataLoader(
        TensorDataset(X_val_permuted, y_val), 
        batch_size=32
    )
    ...
    assert permuted_accuracy > chance_level - 0.1, \
...
def test_multiple_permutations_consistency(make_loader):
    ...
    def create_structured_data_v2(n_samples: int, seed: int):
        ...
        mask4 = (X[:, 0] < -1.0) & (X[:, 1] < -0.5) & (X[:, 2] < -0.5) & (~mask1) & (~mask2) & (~mask3)
        y[mask4] = 4
    ...
    for perm_seed in [111, 222, 333]:  # Test with 3 different permutations
        ...
        perm_train_loader = DataLoader(
            TensorDataset(X_train_perm, y_train), 
            batch_size=32, 
            shuffle=True
        )
        perm_val_loader = DataLoader(
            TensorDataset(X_val_perm, y_val), 
            batch_size=32
        )
