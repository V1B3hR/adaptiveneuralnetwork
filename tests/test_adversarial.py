...
class TestAdversarialInputs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node = AliveLoopNode(
            position=(0, 0), velocity=(0.1, 0.1), initial_energy=10.0, field_strength=1.0, node_id=1
        )
    ...
    def test_excessive_communication_attempts(self):
        ...
        target_node = AliveLoopNode(
            position=(0.5, 0.5), velocity=(0, 0), initial_energy=10.0, node_id=2
        )

        # Try to send many messages rapidly
        sent_count = 0
        for i in range(20):  # Try to send 20 messages (limit is 5)
            signals = self.node.send_signal([target_node], "query", f"message_{i}", urgency=0.5)
            if signals:
...
class TestCommunicationPaths(unittest.TestCase):
    ...
    def test_bidirectional_communication(self):
        ...
        self.assertGreater(
            len(self.node1.signal_history),
            initial_signal_count,
