...
from adaptiveneuralnetwork.core.neuromorphic_v3 import (
    MultiCompartmentNeuron, AdaptiveThresholdNeuron, BurstingNeuron, StochasticNeuron,
    STDPSynapse, MetaplasticitySynapse, HomeostaticScaling, MultiTimescalePlasticity,
    HierarchicalNetwork, DynamicConnectivity, PopulationLayer, RealisticDelays,
    TemporalPatternEncoder, PhaseEncoder, OscillatoryDynamics, SparseDistributedRepresentation
)

# Import V3 configurations
from adaptiveneuralnetwork.core.neuromorphic_v3.advanced_neurons import NeuronV3Config, CompartmentConfig
from adaptiveneuralnetwork.core.neuromorphic_v3.plasticity import STDPConfig, MetaplasticityConfig, HomeostaticConfig
...
from adaptiveneuralnetwork.neuromorphic import (
    Loihi2Backend, SpiNNaker2Backend, GenericV3Backend
)
...
def test_plasticity_mechanisms():
    ...
    meta_synapse = MetaplasticitySynapse(pre_size=8, post_size=4, 
                                       stdp_config=stdp_config, meta_config=meta_config)
    ...
    print(f"  Target vs actual rate: {homeostatic_info['target_rate']:.1f} vs "
...
def test_network_topology():
    ...
    dynamic_conn = DynamicConnectivity(
        pre_size=12, post_size=8, config=topology_config
    )
    ...
    for t in range(50):
        ...
        current, conn_info = dynamic_conn(
            pre_spikes.float(), post_spikes.float(),
            current_time=t * 0.001, plasticity=True
        )
...
def test_temporal_coding():
    ...
    pattern_encoder = TemporalPatternEncoder(
        input_size=20, pattern_size=10, config=temporal_config
    )
    
    for t in range(15):
        input_spikes = torch.rand(2, 20) > 0.92
        patterns, encoding_info = pattern_encoder(
            input_spikes.float(), current_time=t * 0.001
        )
    ...
    for t in range(20):
        ...
        phase_encoded, phase_info = phase_encoder(
            input_spikes.float(), current_time=t * 0.001
        )
    ...
    for t in range(30):
        ...
        oscillator_output, osc_info = oscillatory(
            external_input, current_time=t * 0.001
        )
