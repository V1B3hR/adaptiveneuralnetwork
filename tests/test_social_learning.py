...
from core.social_learning import (
    ConsensusProposal,
    MultiAgentSocialLearningEnvironment,
    SocialLearningAgent,
...
class TestSocialLearningAgent(unittest.TestCase):
    ...
    def test_consensus_evaluation(self):
        ...
        consensus_reached, consensus_strength = self.agent.evaluate_consensus(
            proposal, total_agents=5
        )

        self.assertTrue(
            consensus_reached
        )  # Should reach consensus with 3/4 votes and high confidence
...
class TestMultiAgentSocialLearningEnvironment(unittest.TestCase):
    ...
    def test_complex_social_learning_scenario(self):
        ...
        proposal = self.env.agents[0].propose_consensus(
            content={"adopt_strategy": "efficient_pathfinding"}, confidence=0.9
        )
