...
def run_integration_simulation(system, num_steps=20):
    ...
    for step in range(num_steps):
        ...
        # Get dominant phase
        dominant_phase_name = max(phase_stats.items(), key=lambda x: x[1])[0].replace('_ratio', '').upper()
        dominant_phase = getattr(Phase, dominant_phase_name) if dominant_phase_name in ['ACTIVE', 'SLEEP', 'INTERACTIVE', 'INSPIRED'] else Phase.ACTIVE
        
        network.current_phase = dominant_phase
        phase_history.append(dominant_phase.name)
        
        print(f"Phase: {dominant_phase.name} ({phase_stats[f'{dominant_phase.name.lower()}_ratio']:.2%} dominant)")
        ...
        optimal_precision = precision_mgr.get_optimal_precision(dominant_phase, complexity_score=0.7)
        ...
        print(f"Precision: {optimal_precision.value}, Quantization: {precision_metrics['quantization_strategy']}")
        ...
        print(f"Spike sim: {spike_metrics.spike_rate_hz:.1f} Hz, {spike_metrics.power_consumption_mw:.1f} mW")
    ...
    print(f"Average Performance: {np.mean(performance_history):.3f} ± {np.std(performance_history):.3f}")
...
def demonstrate_multimodal_capabilities():
    ...
    with open('adaptiveneuralnetwork/applications/multimodal_vl.py', 'r') as f:
        content = f.read()
