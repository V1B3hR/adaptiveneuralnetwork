...
class NodeLifecycleManager:
    ...
    def __init__(
        ...
        self.activity_history = torch.zeros(
            (self.num_nodes, self.config.evaluation_window), device=self.device
        )
    ...
    def update_node_metrics(self, contribution_scores: Optional[torch.Tensor] = None) -> None:
        ...
        if activity.shape[0] != self.num_nodes:
            ...
            if activity.shape[0] < self.num_nodes:
                activity = torch.cat(
                    [
                        activity,
                        torch.zeros(self.num_nodes - activity.shape[0], device=activity.device),
                    ]
                )
            else:
        ...
        if energy.shape[0] != self.num_nodes:
            ...
            if energy.shape[0] < self.num_nodes:
                energy = torch.cat(
                    [energy, torch.zeros(self.num_nodes - energy.shape[0], device=energy.device)]
                )
            else:
            ...
            if contrib.shape[0] != self.num_nodes:
                if contrib.shape[0] < self.num_nodes:
                    contrib = torch.cat(
                        [
                            contrib,
                            torch.zeros(self.num_nodes - contrib.shape[0], device=contrib.device),
                        ]
                    )
                else:
    ...
    def assess_node_health(self) -> torch.Tensor:
        ...
        health = torch.full(
            (self.num_nodes,), 0, dtype=torch.int8, device=self.device
        )  # 0 = healthy
    ...
    def identify_pruning_candidates(self) -> torch.Tensor:
        ...
        max_to_prune = min(
            int(current_active * self.config.max_prune_rate), current_active - self.config.min_nodes
        )
    ...
    def identify_healing_candidates(self, current_performance: float) -> Tuple[torch.Tensor, bool]:
        ...
        if needs_healing and self.pruned_nodes.any():
            ...
            if len(pruned_indices) > 0:
                max_to_heal = min(
                    int(self.num_nodes * self.config.max_heal_rate), len(pruned_indices)
                )
    ...
    def heal_nodes(self, node_indices: torch.Tensor) -> Dict[str, Any]:
        ...
        self.node_state.energy[node_indices] = (
            torch.rand(node_indices.sum(), device=self.device) * 0.5 + 0.25
        )  # Random energy between 0.25 and 0.75

        self.node_state.activity[node_indices] = (
            torch.rand(node_indices.sum(), device=self.device) * 0.3 + 0.1
        )  # Random activity between 0.1 and 0.4
    ...
    def should_run_pruning(self) -> bool:
        ...
        return (
            self.step_count > self.config.evaluation_window
            and self.step_count % self.config.pruning_frequency == 0
        )
    ...
    def should_run_healing(self) -> bool:
        ...
        return (
            self.step_count > self.config.evaluation_window
            and self.step_count % self.config.healing_frequency == 0
        )
    ...
    def step(
        ...
        """
        results = {"step": self.step_count, "actions": [], "metrics": {}, "node_health": {}}
        ...
        if current_performance is not None and self.should_run_healing():
            healing_candidates, needs_healing = self.identify_healing_candidates(
                current_performance
            )
        ...
        if activity_flat.shape[0] != self.num_nodes:
            if activity_flat.shape[0] < self.num_nodes:
                activity_flat = torch.cat(
                    [
                        activity_flat,
                        torch.zeros(
                            self.num_nodes - activity_flat.shape[0], device=activity_flat.device
                        ),
                    ]
                )
            else:
        ...
        if energy_flat.shape[0] != self.num_nodes:
            if energy_flat.shape[0] < self.num_nodes:
                energy_flat = torch.cat(
                    [
                        energy_flat,
                        torch.zeros(
                            self.num_nodes - energy_flat.shape[0], device=energy_flat.device
                        ),
                    ]
                )
            else:
        ...
        results["metrics"] = {
            ...
            "mean_activity": (
                activity_flat[self.active_nodes].mean().item() if self.active_nodes.any() else 0
            ),
            "mean_energy": (
                energy_flat[self.active_nodes].mean().item() if self.active_nodes.any() else 0
            ),
...
def create_adaptive_pruning_manager(
    node_state: NodeState, phase_scheduler: Optional[PhaseScheduler] = None, **config_kwargs
