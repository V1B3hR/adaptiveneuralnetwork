...
class AdaptiveProcessor:
    ...
    def should_skip_frame(self, current_latency: float, queue_size: int) -> int:
        ...
        if current_latency > self.config.max_latency_ms:
            skip_frames = min(self.config.max_skip_frames,
                            int(current_latency / self.config.target_latency_ms))
    ...
    def adapt_resolution(self, confidence: float, latency: float) -> int:
        ...
        if (self.consecutive_high_latency >= 3 or
            ...
            new_resolution = max(self.config.min_resolution,
        ...
        elif (self.consecutive_high_latency == 0 and
              self.consecutive_low_confidence == 0 and
              latency < self.config.target_latency_ms * 0.5):
            new_resolution = min(self.config.max_resolution,
                               self.current_resolution + 32)
            if new_resolution != self.current_resolution:
...
class RealTimeInferenceEngine:
    ...
    def submit_frame_sequence(self, sequence: torch.Tensor, frame_info: FrameInfo | None = None):
    ...
    def get_result(self, timeout: float = 0.1) -> InferenceResult | None:
        ...
        while self.is_running:
            try:
                ...
                target_batch_size = self.adaptive_processor.get_batch_size(
                    self.performance_monitor.latencies[-1] if self.performance_monitor.latencies else 0
                )
                ...
                while len(batch_sequences) < target_batch_size and (time.time() - start_batch_time) < 0.05:
                    try:
    ...
    def _run_batch_inference(
        ...
        frame_infos: list[FrameInfo | None],
...
class VideoStreamInference:
    ...
    def __init__(
        ...
        self.inference_engine = RealTimeInferenceEngine(
            model, self.inference_config, self.device
        )
    ...
    def start_stream_inference(self, source: str | int) -> bool:
