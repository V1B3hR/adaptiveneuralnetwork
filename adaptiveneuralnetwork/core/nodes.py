...
class NodeState:
    ...
    def __init__(self, config: NodeConfig):
        ...
        self.hidden_state = torch.zeros(
            1, config.num_nodes, config.hidden_dim,
            dtype=config.dtype, device=self.device
        )
        
        self.energy = torch.ones(
            1, config.num_nodes, config.energy_dim,
            dtype=config.dtype, device=self.device
        ) * 10.0  # Initial energy
        
        self.activity = torch.zeros(
            1, config.num_nodes, config.activity_dim,
            dtype=config.dtype, device=self.device
        )
        
        self.position = torch.randn(
            1, config.num_nodes, config.spatial_dim,
            dtype=config.dtype, device=self.device
        )
        
        # Phase-specific state (will be set by PhaseScheduler)
        self.phase_mask = torch.zeros(
            1, config.num_nodes, 1,
    ...
    def update_activity(self, input_stimulation: torch.Tensor) -> None:
        ...
        self.activity = torch.sigmoid(input_stimulation + self.hidden_state.mean(dim=-1, keepdim=True))
        self.clamp_activity()
