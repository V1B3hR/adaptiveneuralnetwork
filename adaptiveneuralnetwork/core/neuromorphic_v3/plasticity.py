...
class STDPSynapse(nn.Module):
    ...
    def __init__(
        ...
        self.register_parameter(
            "weights", nn.Parameter(torch.full((pre_size, post_size), initial_weight))
        )
    ...
    def forward(
        ...
        self.last_pre_spike = torch.where(pre_spike_mask, current_time, self.last_pre_spike)

        self.last_post_spike = torch.where(post_spike_mask, current_time, self.last_post_spike)
        ...
        if learning:
            # STDP weight updates
            weight_changes = self._calculate_stdp_updates(pre_spikes, post_spikes, current_time, dt)
    ...
    def _calculate_stdp_updates(
        self, pre_spikes: torch.Tensor, post_spikes: torch.Tensor, current_time: float, dt: float
    ) -> torch.Tensor:
        ...
        for b in range(batch_size):
            ...
            for post_idx in post_spike_indices:
                ...
                for pre_idx in pre_spike_indices:
                    ...
                    if 0 < dt_spike <= self.config.max_dt:
                        # Potentiation
                        ltp_amount = self.config.a_plus * torch.exp(
                            torch.tensor(-dt_spike / self.config.tau_plus)
                        )
            ...
            for pre_idx in pre_spike_indices:
                ...
                for post_idx in post_spike_indices:
                    ...
                    if 0 < dt_spike <= self.config.max_dt:
                        # Depression
                        ltd_amount = -self.config.a_minus * torch.exp(
                            torch.tensor(-dt_spike / self.config.tau_minus)
                        )
...
class MetaplasticitySynapse(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "theta", torch.full((post_size,), meta_config.theta_plus)
        )  # Sliding threshold
    ...
    def forward(
        ...
        if self.theta.dim() == 1:
            ...
            self.activity_history = (
                self.activity_history.unsqueeze(0).expand(batch_size, -1, -1).clone()
            )
        ...
        if learning:
            ...
            self.theta = torch.clamp(
                self.theta, self.meta_config.theta_minus, self.meta_config.theta_plus * 2
            )
        ...
        if learning:
            ...
            self.stdp_synapse.weights.data = torch.clamp(
                new_weights, self.stdp_config.w_min, self.stdp_config.w_max
            )

        plasticity_info = {
            "weight_changes": (
                modulated_changes if learning else torch.zeros_like(self.stdp_synapse.weights)
            ),
...
class HomeostaticScaling(nn.Module):
    ...
    def __init__(self, num_neurons: int, config: HomeostaticConfig):
        super().__init__()
    ...
    def forward(
        ...
        if apply_scaling and window_duration >= self.config.scaling_window:
            ...
            self.scaling_factors = torch.clamp(
                new_scaling, self.config.min_scaling, self.config.max_scaling
            )
...
class MultiTimescalePlasticity(nn.Module):
    ...
    def __init__(
        ...
        self.stdp_synapse = STDPSynapse(pre_size, post_size, stdp_config)
        self.metaplastic_synapse = MetaplasticitySynapse(
            pre_size, post_size, stdp_config, meta_config
        )
