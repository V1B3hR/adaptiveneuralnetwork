...
class PopulationLayer(nn.Module):
    ...
    def __init__(
        ...
        neuron_config: Optional[NeuronV3Config] = None,
        ...
        if lateral_inhibition:
            ...
            self.register_buffer("inhibition_mask", torch.eye(population_size) == 0)
    ...
    def forward(
        ...
        if self.lateral_inhibition:
            ...
            lateral_inhibition = torch.matmul(
                prev_activity, self.lateral_weights * self.inhibition_mask
            )
...
class RealisticDelays(nn.Module):
    ...
    def __init__(self, source_size: int, target_size: int, config: TopologyConfig):
        super().__init__()
        ...
        self.register_buffer("spike_buffer", torch.zeros(max_delay_steps, source_size))
        ...
        if self.config.delay_scaling == "distance":
            ...
            for i in range(self.source_size):
                for j in range(self.target_size):
                    ...
                    normalized_distance = distance / torch.sqrt(
                        torch.tensor(2.0)
                    )  # Max distance in unit square
            ...
            delays = (
                torch.rand(self.source_size, self.target_size)
                * (self.config.max_delay - self.config.min_delay)
    ...
    def forward(self, input_spikes: torch.Tensor, dt: Optional[float] = None) -> torch.Tensor:
        """
...
class DynamicConnectivity(nn.Module):
    ...
    def __init__(
        ...
        initial_connectivity: Optional[torch.Tensor] = None,
    ):
        super().__init__()
    ...
    def _generate_initial_connectivity(self) -> torch.Tensor:
        ...
        connectivity = (
            torch.rand(self.pre_size, self.post_size) < self.config.connection_probability
        )
    ...
    def forward(
        ...
        current_correlation = torch.outer(pre_spikes.mean(dim=0), post_spikes.mean(dim=0))
    ...
    def _form_synapses(self):
        ...
        formation_candidates = (avg_correlation > correlation_threshold) & (
            self.connectivity_mask == 0
        )
...
class HierarchicalNetwork(nn.Module):
    ...
    def __init__(
        self,
        config: TopologyConfig,
        neuron_configs: Optional[List[NeuronV3Config]] = None,
        ...
        for i in range(config.num_layers - 1):
            ...
            ff_conn = DynamicConnectivity(config.layer_sizes[i], config.layer_sizes[i + 1], config)
            self.feedforward_connections.append(ff_conn)

            # Feedback connections (layer i+1 -> layer i)
            fb_conn = DynamicConnectivity(config.layer_sizes[i + 1], config.layer_sizes[i], config)
            self.feedback_connections.append(fb_conn)

            # Delays for both directions
            ff_delay = RealisticDelays(config.layer_sizes[i], config.layer_sizes[i + 1], config)
            fb_delay = RealisticDelays(config.layer_sizes[i + 1], config.layer_sizes[i], config)
        ...
        logger.info(
            f"Created hierarchical network: {config.num_layers} layers {config.layer_sizes}"
        )
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if layer_idx < len(self.layers) - 1:
                ...
                ff_current, ff_info = ff_conn(
                    layer_output,
                    (
                        torch.zeros_like(layer_outputs[layer_idx + 1])
                        if layer_idx + 1 < len(layer_outputs)
                        else torch.zeros(batch_size, self.config.layer_sizes[layer_idx + 1])
                    ),
                    current_time,
                    dt,
                )
        ...
        for layer_idx in range(len(self.layers) - 2, -1, -1):
            ...
            fb_current, fb_info = fb_conn(
                higher_layer_output, layer_outputs[layer_idx], current_time, dt
            )
        ...
        for i in range(len(layer_outputs) - 1):
            ...
            corr = torch.corrcoef(
                torch.stack([layer_outputs[i].mean(dim=0), layer_outputs[i + 1].mean(dim=0)])
            )[0, 1]
        ...
        network_states = {
            ...
            "feedforward_info": [
                conn.connectivity_mask.sum().item() for conn in self.feedforward_connections
            ],
            "feedback_info": [
                conn.connectivity_mask.sum().item() for conn in self.feedback_connections
            ],
        }
