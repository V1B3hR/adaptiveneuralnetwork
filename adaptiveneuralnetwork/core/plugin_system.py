...
class PhasePlugin(ABC):
    ...
    def apply_phase_logic(
        self, node_state: NodeState, phase_scheduler: PhaseScheduler, step: int, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def apply_phase_logic(
        ...
        if creative_nodes.any():
            ...
            results["modifications"].append(
                f"Applied creativity boost {boost} to {creative_nodes.sum()} nodes"
            )
        ...
        if self.config["exploration_noise"] > 0:
            ...
            results["modifications"].append(
                f"Added exploration noise (std={self.config['exploration_noise']})"
            )

        # Track creative metrics
        results["metrics"] = {
            "creative_nodes": creative_nodes.sum().item(),
            "mean_creative_activity": (
                node_state.activity[creative_nodes].mean().item() if creative_nodes.any() else 0
            ),
...
class ConsolidationPhase(PhasePlugin):
    ...
    def apply_phase_logic(
        ...
        results["metrics"] = {
            ...
            "consolidated_nodes": (
                consolidation_nodes.sum().item() if "consolidation_nodes" in locals() else 0
            ),
        }
...
class PluginManager:
    ...
    def register_plugin(
        self, plugin: PhasePlugin, phase_id: Optional[int] = None, replace_existing: bool = False
    ) -> int:
        """
        ...
        if plugin.name in self.plugins and not replace_existing:
            raise ValueError(
                f"Plugin '{plugin.name}' already registered. Use replace_existing=True to override."
            )
    ...
    def apply_plugin_phases(
        ...
        for plugin_name in self.active_plugins:
            ...
            if nodes_in_phase or plugin.config.get("always_apply", False):
                try:
                    results = plugin.apply_phase_logic(node_state, phase_scheduler, step, **kwargs)
                    all_results["plugins"][plugin_name] = results

                    # Update summary
                    if "modifications" in results:
                        all_results["summary"]["total_modifications"] += len(
    ...
    def get_extended_transition_matrix(
        self, base_transitions: torch.Tensor, num_base_phases: int = 4
    ) -> torch.Tensor:
        """
...
class PluginAwarePhaseScheduler(PhaseScheduler):
    ...
    def step(self, node_state: NodeState, current_step: int = None, **kwargs) -> Dict[str, Any]:
        ...
        step_results = super().step(
            node_state.energy, node_state.activity, getattr(node_state, "anxiety", None)
        )
        ...
        if current_step is not None:
            ...
            if isinstance(step_results, dict):
                ...
                step_results = {"phase_transitions": step_results, "plugins": plugin_results}
