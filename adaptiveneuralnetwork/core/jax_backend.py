...
if JAX_AVAILABLE:
    ...
    class JAXNodeState:
        ...
        def __init__(self, config: JAXNodeConfig, key: Optional[jax.random.PRNGKey] = None):
            if not JAX_AVAILABLE:
                raise ImportError(
                    "JAX is not available. Please install JAX to use the JAX backend."
                )
        ...
        def _initialize_state(self, key: jax.random.PRNGKey) -> Dict[str, jnp.ndarray]:
            ...
            return {
                "hidden_state": random.normal(
                    keys[0], (1, self.config.num_nodes, self.config.hidden_dim)
                ),
        ...
        def expand_batch(
            self, state: Dict[str, jnp.ndarray], batch_size: int
...
if JAX_AVAILABLE:
    ...
    def compute_phase_influence(
        phase_state: jnp.ndarray, phase_weights: jnp.ndarray
    ) -> jnp.ndarray:
...
if JAX_AVAILABLE:
    ...
    class JAXAdaptiveDynamics(nn.Module):
        ...
        def __call__(
            ...
            # Update activity
            activity_features = jnp.concatenate(
                [new_hidden[:, :2], flat_energy], axis=-1
            )  # Use first 2 dims of hidden
    ...
    class JAXAdaptiveModel(nn.Module):
        ...
        def setup(self):
            ...
            self.dynamics = JAXAdaptiveDynamics(
                hidden_dim=self.config.hidden_dim, config=self.config
            )
            self.output_projection = nn.Dense(self.output_dim)

        def __call__(
            self, x: jnp.ndarray, state: Dict[str, jnp.ndarray], training: bool = True
    ...
    class JAXTrainingState:
        ...
        def __init__(
            self, model: JAXAdaptiveModel, learning_rate: float = 0.001, optimizer: str = "adam"
        ):
            if not JAX_AVAILABLE:
                raise ImportError(
                    "JAX is not available. Please install JAX to use the JAX backend."
        ...
        def create_train_state(
            self, key: jax.random.PRNGKey, input_shape: Tuple[int, ...]
            ...
            dummy_state = {
                "hidden_state": jnp.ones(
                    (1, self.model.config.num_nodes, self.model.config.hidden_dim)
                ),
...
if JAX_AVAILABLE:
    ...
    def train_step(
        ...
        node_state: Dict[str, jnp.ndarray],
    ) -> Tuple[train_state.TrainState, float, Dict[str, jnp.ndarray]]:
        """
        ...
        (loss, (outputs, new_node_state)), grads = jax.value_and_grad(loss_fn, has_aux=True)(
            state.params
        )
