...
class LanguageEncoder(nn.Module):
    ...
    def __init__(self, config: VisionLanguageConfig):
        ...
        self.position_embedding = nn.Embedding(
            config.max_sequence_length, config.language_feature_dim
        )
    ...
    def forward(
        self, text_tokens: torch.Tensor, attention_mask: Optional[torch.Tensor] = None
        ...
        positions = (
            torch.arange(seq_len, device=text_tokens.device).unsqueeze(0).expand(batch_size, -1)
        )
        ...
        if self.config.language_encoder_type == "transformer":
            ...
            if attention_mask is not None:
                ...
                sequence_features = masked_encoded.sum(dim=1) / attention_mask.sum(
                    dim=1, keepdim=True
                )
            else:
            ...
            encoded, (hidden, _) = (
                ...
                if self.config.language_encoder_type == "lstm"
                else self.encoder(embeddings)
            )
            # Use final hidden state (bidirectional, so concatenate)
            if isinstance(hidden, tuple):
                hidden = hidden[0]  # LSTM returns (hidden, cell)
            sequence_features = (
                hidden[-2:].transpose(0, 1).contiguous().view(batch_size, -1)
...
class CrossModalFusion(nn.Module):
    ...
    def _build_bilinear_fusion(self):
        ...
        return nn.Bilinear(self.config.fusion_dim, self.config.fusion_dim, self.config.fusion_dim)

    def _build_gated_fusion(self):
        """Build gated fusion."""
        return nn.Sequential(
            nn.Linear(self.config.fusion_dim * 2, self.config.fusion_dim), nn.Sigmoid()
        )
...
class VisionLanguageModel(nn.Module):
    ...
    def _build_neuromorphic_processor(self):
        ...
        topology_config = TopologyConfig(
            ...
            layer_sizes=[
                ...
                self.config.fusion_dim // 4,
            ],
            connection_probability=0.1,
            enable_dynamic_connectivity=True,
        )

        neuron_config = NeuronV3Config(threshold_adaptation_rate=0.1, target_spike_rate=20.0)
    ...
    def forward(
        ...
        neuromorphic_output, neuromorphic_states = self.neuromorphic_processor(
...
class ImageCaptioningHead(nn.Module):
    ...
    def forward(
        ...
        target = torch.zeros(
            batch_size, max_len, self.config.fusion_dim, device=fused_features.device
        )
...
class SceneGraphHead(nn.Module):
    ...
    def forward(
        ...
        features_paired = torch.cat(
            [features_expanded, fused_features.unsqueeze(0).expand(batch_size, -1, -1)], dim=-1
        )

        relationship_logits = self.relationship_classifier(features_paired)

        return {"objects": object_logits, "relationships": relationship_logits}
...
class SpatialAttention(nn.Module):
    ...
    def forward(
        self, features: torch.Tensor, attention_maps: Optional[torch.Tensor] = None
