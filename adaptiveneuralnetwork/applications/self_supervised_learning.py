...
class SignalPredictor(nn.Module):
    ...
    def __init__(self, config: SelfSupervisedConfig, input_dim: int):
        ...
        self.context_encoder = nn.LSTM(input_dim, config.hidden_dim, batch_first=True)
    ...
    def forward(self, context: torch.Tensor) -> torch.Tensor:
        ...
        predictions = predictions.view(-1, self.config.prediction_horizon, self.input_dim)

        return predictions

    def compute_prediction_loss(self, context: torch.Tensor, future: torch.Tensor) -> torch.Tensor:
...
class SelfSupervisedLearningSystem(nn.Module):
    ...
    def train_step(
        self, signals: torch.Tensor, labels: Optional[torch.Tensor] = None
    ) -> Dict[str, float]:
        """
