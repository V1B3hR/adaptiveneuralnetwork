...
class TimeSeriesAnalyzer:
    ...
    def detect_significant_events(self) -> List[Tuple[int, float, str]]:
        ...
        rolling_std = np.array(
            [np.std(history_array[max(0, i - 4) : i + 1]) for i in range(len(history_array))]
        )
...
class DynamicPriorityBuffer:
    ...
    def sample(
        ...
        importance_weights = (
            1.0 / (self.stored_samples * sampling_probabilities)
        ) ** self.config.importance_sampling_weight
        ...
        return (
            ...
            sampled_labels,
            sampled_tasks,
            torch.tensor(importance_weights, dtype=torch.float32),
            indices,
        )
...
class EventDrivenLearningSystem:
    ...
    def get_memory_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_importance": self.memory_buffer.importance_scores[
                : self.memory_buffer.stored_samples
            ]
            .mean()
            .item(),
            "max_importance": self.memory_buffer.importance_scores[
                : self.memory_buffer.stored_samples
            ]
            .max()
            .item(),
