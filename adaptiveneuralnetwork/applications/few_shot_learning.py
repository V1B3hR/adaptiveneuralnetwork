...
from ..core.neuromorphic_v3.plasticity import MetaplasticityConfig, STDPConfig
...
class PrototypicalMemory(nn.Module):
    ...
    def __init__(self, feature_dim: int, memory_dim: int, temporal_config: TemporalConfig):
        super().__init__()
    ...
    def retrieve_similar_prototypes(
        self, query_features: torch.Tensor, top_k: int = 5
    ...
    def clear_prototypes(self) -> None:
        ...
        self.prototype_labels = torch.zeros(
            0, dtype=torch.long, device=self.prototype_labels.device
        )
...
class RapidPlasticityModule(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, config: FewShotLearningConfig):
        super().__init__()
    ...
    def forward(
        self, input_spikes: torch.Tensor, target_spikes: torch.Tensor, learning: bool = True
    ...
    def rapid_adapt(
        self, support_inputs: torch.Tensor, support_targets: torch.Tensor, num_steps: int = 5
    ) -> Dict[str, float]:
        """Perform rapid adaptation on support set using fast plasticity."""
        adaptation_stats = {"weight_changes": 0.0, "adaptation_strength": 0.0}
        ...
        for step in range(num_steps):
            ...
            self.meta_synapses.stdp_synapse.weights.data += (
                weight_update * self.config.fast_learning_rate
            )
...
class FewShotLearningSystem(nn.Module):
    ...
    def __init__(self, config: FewShotLearningConfig):
        ...
        if config.enable_temporal_encoding:
            temporal_config = TemporalConfig(pattern_window=0.1, max_pattern_length=10)
        ...
        if config.enable_rapid_plasticity:
            self.rapid_plasticity = RapidPlasticityModule(config.feature_dim, config.n_way, config)
        else:
        ...
        logger.info(
            f"Initialized few-shot learning system: {config.n_way}-way, {config.k_shot}-shot"
        )
    ...
    def forward(
        ...
        if self.rapid_plasticity is not None:
            ...
            support_spikes = (
                torch.rand_like(support_patterns) < torch.sigmoid(support_patterns)
            ).float()
        ...
        distances = torch.norm(
            query_expanded - prototypes_expanded, dim=2
    ...
    def adaptation_analysis(
        ...
        if self.rapid_plasticity is not None:
            ...
            # Perform adaptation
            support_spikes = (
                torch.rand_like(initial_features) < torch.sigmoid(initial_features)
            ).float()
            adaptation_stats = self.rapid_plasticity.rapid_adapt(
                support_spikes, support_y, self.config.fast_adaptation_steps
            )
            ...
            analysis.update(
                {
                    ...
                    "weight_change_ratio": (
                        torch.norm(final_weights - initial_weights) / torch.norm(initial_weights)
                    ).item(),
        ...
        if self.prototypical_memory is not None:
            unique_classes = torch.unique(support_y)
            memory_stats = {"num_classes": len(unique_classes), "prototype_similarity": {}}
            ...
            for i, class_i in enumerate(unique_classes):
                ...
                for j, class_j in enumerate(unique_classes):
                    if i < j:  # Only compute upper triangle
                        ...
                        memory_stats["prototype_similarity"][
                            f"{class_i.item()}_{class_j.item()}"
                        ] = similarity
