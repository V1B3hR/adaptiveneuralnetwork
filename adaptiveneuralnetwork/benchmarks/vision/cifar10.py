...
from ...training.datasets import load_cifar10, load_cifar10_corrupted, CIFAR10Corrupted
...
class CIFAR10Benchmark:
    ...
    def __init__(
        ...
        logger.info(f"Initialized CIFAR-10 benchmark with {sum(p.numel() for p in self.model.parameters())} parameters")
    ...
    def run_standard_benchmark(
        ...
        logger.info(f"Standard CIFAR-10 benchmark completed. "
    ...
    def run_robustness_benchmark(
        ...
        if corruption_types is None:
            corruption_types = ['gaussian_noise', 'brightness', 'contrast', 'pixelate', 'gaussian_blur']
        ...
        clean_loader = load_cifar10(batch_size=batch_size, root=self.data_root, num_workers=num_workers)[1]
            ...
            for severity in severities:
                ...
                try:
                    ...
                    robustness_results['corruption_results'][corruption_type][severity] = {
                        ...
                        'relative_accuracy': corrupted_accuracy / clean_accuracy if clean_accuracy > 0 else 0.0
                    }
                    
                    logger.info(f"  Accuracy: {corrupted_accuracy:.4f}, "
                               f"Error: {corruption_error:.4f}")
        ...
        if num_evaluations > 0:
            ...
            robustness_results['relative_robustness'] = 1.0 - (robustness_results['mean_corruption_error'] / clean_accuracy) if clean_accuracy > 0 else 0.0
        ...
        logger.info(f"Robustness benchmark completed. "
...
if __name__ == "__main__":
    ...
    if 'robustness' in results:
        print(f"CIFAR-10 Robustness: {results['robustness']['robustness_results']['relative_robustness']:.4f}")