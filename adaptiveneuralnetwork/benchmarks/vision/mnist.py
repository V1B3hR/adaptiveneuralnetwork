...
def run_mnist_benchmark(
    config: AdaptiveConfig | None = None, subset_size: int | None = None, save_results: bool = True
) -> dict[str, Any]:
    """
    ...
    if subset_size is not None:
        train_loader, test_loader = load_mnist_subset(
            batch_size=config.batch_size, subset_size=subset_size
        )
...
def quick_mnist_test(num_epochs: int = 1, subset_size: int = 1000) -> dict[str, Any]:
    ...
    return run_mnist_benchmark(config=config, subset_size=subset_size, save_results=True)
