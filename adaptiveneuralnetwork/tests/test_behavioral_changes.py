...
class TestProactiveInterventionBehavior:
    ...
    def test_anxiety_intervention_enabled_vs_disabled(self):
        ...
        node_enabled = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config_enabled
        )
        node_disabled = AliveLoopNode(
            position=[1.0, 1.0], velocity=[0.0, 0.0], node_id=2, config=config_disabled
        )
    ...
    def test_trend_window_variation_affects_slope_estimation(self):
        ...
        node_small = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config_small
        )
        node_large = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=2, config=config_large
        )
    ...
    def test_max_help_signals_cap_respected(self):
        ...
        node = AliveLoopNode(position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config)
    ...
    def test_anxiety_threshold_affects_intervention_trigger(self):
        ...
        node_low = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config_low
        )
        node_high = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=2, config=config_high
        )
...
class TestAttackResilienceBehavior:
    ...
    def test_frequency_hopping_can_be_toggled(self):
        ...
        node_disabled = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config_disabled
        )
        node_enabled = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=2, config=config_enabled
        )
    ...
    def test_max_drain_per_attacker_ratio_caps_damage(self):
        ...
        node_strict = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config_strict
        )
        node_lenient = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=2, config=config_lenient
        )
        ...
        assert (
            node_strict.config.attack_resilience.max_drain_per_attacker_ratio
...
class TestTrustManipulationDetection:
    ...
    def test_trust_growth_rate_limit_affects_detection(self):
        ...
        assert (
            node_strict.config.attack_resilience.trust_growth_rate_limit
    ...
    def test_rapid_trust_threshold_affects_detection(self):
        ...
        config_sensitive.attack_resilience.rapid_trust_threshold = (
            1.0  # Low threshold = more sensitive
        )

        config_tolerant = AdaptiveNeuralNetworkConfig()
        config_tolerant.attack_resilience.rapid_trust_threshold = (
            5.0  # High threshold = less sensitive
        )

        node_sensitive = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config_sensitive
        )
        node_tolerant = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=2, config=config_tolerant
        )
        ...
        assert (
            node_sensitive.config.attack_resilience.rapid_trust_threshold
...
class TestEnvironmentAdaptationBehavior:
    ...
    def test_stress_thresholds_affect_adaptation(self):
        ...
        assert (
            node_sensitive.config.environment_adaptation.stress_threshold_low
            < node_tolerant.config.environment_adaptation.stress_threshold_low
        )
        assert (
            node_sensitive.config.environment_adaptation.stress_threshold_high
    ...
    def test_adaptation_rate_affects_learning_speed(self):
        ...
        node_slow = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=1, config=config_slow
        )
        node_fast = AliveLoopNode(
            position=[0.0, 0.0], velocity=[0.0, 0.0], node_id=2, config=config_fast
        )
        ...
        assert (
            node_fast.config.environment_adaptation.adaptation_rate
