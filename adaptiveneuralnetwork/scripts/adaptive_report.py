...
class AdaptiveReportGenerator:
    ...
    def extract_metric_value(
        self, data: Dict[str, Any], metric_path: str, default: str = "N/A"
    ) -> str:
    ...
    def generate_robustness_adversarial_summary(self) -> str:
        ...
        if "enhanced_robustness_results" in self.results:
            ...
            # Overall metrics
            overall_score = self.extract_metric_value(
                robustness_data, "overall_robustness_score", "N/A"
            )
            total_duration = self.extract_metric_value(
                robustness_data, "total_duration_seconds", "N/A"
            )
            ...
            if "scenario_validation" in robustness_data:
                ...
                scenarios_tested = self.extract_metric_value(
                    scenario_data, "scenarios_tested", "N/A"
                )
                scenarios_passed = self.extract_metric_value(
                    scenario_data, "scenarios_passed", "N/A"
                )
                ...
                if scenarios_tested != "N/A" and scenarios_passed != "N/A":
                    try:
                        ...
                        summary_lines.append(
                            f"- **Scenario Validation**: {scenarios_passed}/{scenarios_tested} ({pass_rate:.1f}% passed)"
                        )
                    except (ValueError, ZeroDivisionError):
                        summary_lines.append(
                            f"- **Scenario Validation**: {scenarios_passed}/{scenarios_tested}"
                        )

            # Relative robustness if available
            relative_robustness = self.extract_metric_value(
                robustness_data, "relative_robustness", None
            )
        ...
        if "adversarial_results" in self.results:
            ...
            # Key metrics
            resilience_score = self.extract_metric_value(
                adversarial_data, "adversarial_resilience_score", "N/A"
            )
            ...
            avg_degradation = self.extract_metric_value(
                adversarial_data, "average_performance_degradation", "N/A"
            )
            ...
            if tests_passed != "N/A" and total_tests != "N/A":
                try:
                    ...
                    summary_lines.append(
                        f"- **Tests Passed**: {tests_passed}/{total_tests} ({pass_rate:.1f}%)"
                    )
                except (ValueError, ZeroDivisionError):
            ...
            adversarial_retention = self.extract_metric_value(
                adversarial_data, "adversarial_retention", None
            )
    ...
    def generate_phase_sparsity_snapshot(self) -> str:
        ...
        for result_name, result_data in self.results.items():
            if isinstance(result_data, dict):
                ...
                active_node_ratio = self.extract_metric_value(
                    result_data, "active_node_ratio", None
                )
                if active_node_ratio:
                    if not found_metrics:
                        ...
                        found_metrics = True
                    snapshot_lines.append(
                        f"- **Active Node Ratio** ({result_name}): {active_node_ratio}"
                    )

                # Look for active_phase_ratio
                active_phase_ratio = self.extract_metric_value(
                    result_data, "active_phase_ratio", None
                )
                if active_phase_ratio:
                    if not found_metrics:
                        ...
                        found_metrics = True
                    snapshot_lines.append(
                        f"- **Active Phase Ratio** ({result_name}): {active_phase_ratio}"
                    )
    ...
    def generate_full_report(self) -> str:
        ...
        if not self.results:
            report_lines.append(
                "⚠️ **No benchmark data found** - Please run benchmarks to generate report content."
            )
            report_lines.append("")
        ...
        report_lines.append(
            f"This report consolidates metrics from {total_files} JSON artifact(s):"
        )
        ...
        report_lines.append("")
        report_lines.append(
            "*This report was automatically generated by the `adaptive-report` CLI tool.*"
        )
        report_lines.append("")
