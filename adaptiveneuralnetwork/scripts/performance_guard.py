...
class PerformanceGuard:
    ...
    def extract_performance_metric(self, data: Dict[str, Any], metric_paths: list) -> Optional[float]:
    ...
    def check_performance_regression(self, tolerance_percent: float = 10.0) -> tuple[bool, str]:
        ...
        if performance_delta_percent > tolerance_percent:
            message = (f"Performance regression detected: JAX is {performance_delta_percent:.1f}% "
                      f"slower than PyTorch (tolerance: {tolerance_percent}%)")
            return True, message
        
        message = (f"Performance check passed: JAX delta is {performance_delta_percent:.1f}% "
