...
from core.video_inference import InferenceConfig, InferenceResult, RealTimeInferenceEngine
...
class VideoBenchmark:
    ...
    def run_benchmark(self, source: str, duration: Optional[float] = None) -> Dict[str, Any]:
        ...
        inference_engine = RealTimeInferenceEngine(self.model, inference_config, self.device)
...
Examples:
  ...
  python adaptive_video_benchmark.py --source webcam --model hybrid --batch-size 4 --target-fps 20
    ...
    parser.add_argument(
        "--max-frames", type=int, default=1000, help="Maximum frames to process (default: 1000)"
    )

    parser.add_argument(
        "--duration", type=float, help="Maximum duration in seconds (overrides --max-frames)"
    )
