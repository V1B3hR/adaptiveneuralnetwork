            ...
            for metric_name, path in sparsity_paths:
                ...
                if value is not None:
                    sparsity_metrics.append(
                        {"source": source_name, "metric": metric_name, "value": value, "path": path}
                    )
    ...
    def check_sparsity_health(
        ...
        for metric in sparsity_metrics:
            ...
            # Check for collapse to too dense (network not adaptive)
                ...
                healthy_metrics.append(f"{metric_name} ({source}) = {value:.3f} (healthy range)")
...
def main() -> None:
    ...
    is_violation, message = guard.check_sparsity_health(args.min_activity, args.max_activity)
