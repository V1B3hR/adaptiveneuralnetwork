...
from core.video_inference import VideoStreamInference, InferenceConfig, InferenceResult
            ...
            if labels_path.suffix == '.json':
                with open(labels_path) as f:
                    ...
                    if isinstance(labels_data, dict):
                        ...
                        return [labels_data[str(i)] for i in sorted(int(k) for k in labels_data.keys())]
                    else:
                        return labels_data
            else:
    ...
    def _display_stats(self, stats: Dict[str, Any]):
        ...
        if 'video_stream' in stats:
            ...
            if 'buffer_sizes' in vs:
                buf = vs['buffer_sizes']
                print(f"Buffer sizes - Frames: {buf.get('frames', 0)}, Sequences: {buf.get('sequences', 0)}")
    ...
    def get_final_report(self) -> Dict[str, Any]:
        ...
        report = {
            'summary': {
                ...
                'total_time_seconds': self.results_history[-1].timestamp - self.results_history[0].timestamp,
...
def create_output_handler(output_type: str, config: Dict[str, Any]):
    ...
    if output_type == 'json_log':
        ...
        def csv_handler(result: InferenceResult):
            ...
            with open(output_file, 'a') as f:
                f.write(f'{result.timestamp},{result.frame_id},{result.confidence},'
...
Examples:
  ...
  python adaptive_live_infer.py --source webcam --weights model.pth --labels imagenet_labels.txt
    ...
    parser.add_argument(
        '--labels',
        type=str,
        help='Path to class labels file (JSON or text format)'
    )
    ...
    parser.add_argument(
        '--output-format',
        choices=['json', 'csv'],
        help='Output format for logging results'
    )
    
    parser.add_argument(
        '--output-file',
        type=str,
        help='Output file path (default: inference_log.{format})'
    )
    
    parser.add_argument(
        '--report-file',
        type=str,
        help='Final report output file (JSON format)'
    )
    ...
    parser.add_argument(
        '--quiet',
        action='store_true',
        help='Reduce output (no statistics display)'
    )
