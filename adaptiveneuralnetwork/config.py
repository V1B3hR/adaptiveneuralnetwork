...
class AdaptiveNeuralNetworkConfig:
    ...
    proactive_interventions: ProactiveInterventionsConfig = field(
        default_factory=ProactiveInterventionsConfig
    )
    ...
    def _validate_and_clamp(self):
        ...
        if self.trend_analysis.window < 1:
            ...
            warnings.warn(
                "trend_analysis.window > 100 may cause performance issues, clamping to 100"
            )
        ...
        if self.rolling_history.max_len < 5:
            warnings.warn(
                "rolling_history.max_len should be >= 5 for meaningful trends, clamping to 5"
            )
            self.rolling_history.max_len = 5
        elif self.rolling_history.max_len > 1000:
            warnings.warn(
                "rolling_history.max_len > 1000 may cause memory issues, clamping to 1000"
            )
            self.rolling_history.max_len = 1000

        # Validate attack resilience
        self.attack_resilience.energy_drain_resistance = max(
            0.0, min(1.0, self.attack_resilience.energy_drain_resistance)
        )
        self.attack_resilience.max_drain_per_attacker_ratio = max(
            0.01, min(1.0, self.attack_resilience.max_drain_per_attacker_ratio)
        )
        self.attack_resilience.jamming_detection_sensitivity = max(
            0.0, min(1.0, self.attack_resilience.jamming_detection_sensitivity)
        )
    ...
    def _log_applied_config(self):
        ...
        logger.info(
            f"  Attack resilience - energy drain resistance: {self.attack_resilience.energy_drain_resistance}"
        )
        logger.info(
            f"  Attack resilience - signal redundancy level: {self.attack_resilience.signal_redundancy_level}"
        )
        logger.info(
            f"  Attack resilience - frequency hopping: {self.attack_resilience.frequency_hopping_enabled}"
        )
    ...
    def from_env(cls, prefix: str = "ANN_") -> "AdaptiveNeuralNetworkConfig":
        ...
        env_mappings = {
            ...
            f"{prefix}ENERGY_DRAIN_RESISTANCE": (
                "attack_resilience",
    ...
    def log_event(self, event_type: str, message: str, **kwargs):
        ...
        if should_log:
            log_data = {"event_type": event_type, "message": message, **kwargs}
