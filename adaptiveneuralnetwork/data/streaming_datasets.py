    ...
    def __len__(self) -> int:
        ...
        if self._length is None:
            ...
            if isinstance(self.data_source, (str, Path)):
                ...
                if data_path.is_dir():
                    # Count files
                    self._length = len(list(data_path.glob("*.json"))) + len(
                        list(data_path.glob("*.pkl"))
                    )
                else:
...
class WebDatasetWrapper(UnifiedDatasetInterface):
    ...
    def __init__(
        ...
        config: StreamingConfig,
        decode_handlers: Optional[Dict[str, Callable]] = None,
    ):
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        raise NotImplementedError(
            "WebDataset doesn't support direct indexing. Use stream() instead."
        )
...
class HuggingFaceDatasetWrapper(UnifiedDatasetInterface):
    ...
    def _load_dataset(self):
        ...
        self._dataset = self.load_dataset(
            self.dataset_name, split=self.split, streaming=self.streaming, **self.dataset_kwargs
        )
    ...
    def __len__(self) -> int:
        ...
        if hasattr(self._dataset, "__len__"):
            ...
            return (
                getattr(self._dataset, "info", {})
                .get("splits", {})
                .get(self.split, {})
                .get("num_examples", 10000)
            )
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        if self.streaming:
            raise NotImplementedError(
                "Streaming HF datasets don't support direct indexing. Use stream() instead."
            )
...
class UnifiedDatasetManager:
    ...
    def create_webdataset(
        self, urls: Union[str, List[str]], config: Optional[StreamingConfig] = None, **kwargs
    ) -> WebDatasetWrapper:
    ...
    def create_huggingface_dataset(
        self, dataset_name: str, config: Optional[StreamingConfig] = None, **kwargs
...
class MultiDatasetWrapper(UnifiedDatasetInterface, IterableDataset):
    ...
    def __init__(
        self, datasets: List[UnifiedDatasetInterface], sampling_strategy: str = "round_robin"
    ):
...
def quick_stream_dataset(
    data_source: Union[str, Path, Callable], batch_size: int = 32, shuffle: bool = True, **kwargs
) -> DataLoader:
...
def quick_huggingface_stream(
    dataset_name: str, batch_size: int = 32, split: str = "train", **kwargs
) -> DataLoader:
