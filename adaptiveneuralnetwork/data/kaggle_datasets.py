...
def load_annomi_dataset(
    ...
    return EssayDataset(
        texts=texts, labels=binary_labels, vocab_size=vocab_size, max_length=max_length
    )
...
def load_pos_tagging_dataset(
    ...
    logger.info(
        f"Created datasets - Train: {len(datasets['train'])}, "
...
class POSDataset:
    ...
    def get_batch(self, indices: List[int]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tags": batch_tags,
            "attention_mask": [
                [1 if i < length else 0 for i in range(max_len)] for length in batch_lengths
            ],
        }
...
def _load_pos_data(data_path: str) -> Tuple[List[List[str]], List[List[str]]]:
    ...
    logger.info(
        f"Loaded {len(sentences)} sentences with {sum(len(s) for s in sentences)} total tokens"
    )
    return sentences, all_tags


def _filter_pos_tokens(
    sentences: List[List[str]], all_tags: List[List[str]], min_length: int, filter_punct: bool
...
def _compute_pos_statistics(
    sentences: List[List[str]], all_tags: List[List[str]]
) -> Dict[str, Any]:
...
def _build_pos_vocabularies(
    sentences: List[List[str]], all_tags: List[List[str]], vocab_size: int
...
def create_text_classification_dataset(
    texts: List[str], labels: List[str], vocab_size: int = 10000, max_length: int = 512
) -> EssayDataset:
    """
...
def _find_column(data: pd.DataFrame, possible_names: List[str]) -> str:
    ...
    logger.warning(
        f"Could not find column matching {possible_names}. Using first column: {columns[0]}"
    )
    return columns[0]
...
def load_vr_driving_dataset(
    ...
    if target_column not in df.columns:
        target_column = _find_column(
            df,
            ["performance", "score", "rating", "quality", "result", "outcome", "label", "target"],
        )
    ...
    if feature_columns:
        ...
        feature_texts = df.apply(
            lambda x: " ".join(
                [f"{col}:{val}" for col, val in x.items() if col != target_column and pd.notna(val)]
            ),
            axis=1,
        ).tolist()
    ...
    return create_text_classification_dataset(
        feature_texts, [str(t) for t in targets], vocab_size, max_length
    )
...
def load_autvi_dataset(
    ...
    if target_column not in df.columns:
        target_column = _find_column(
            df, ["inspection_result", "result", "pass_fail", "status", "outcome", "label", "target"]
        )
...
def load_digakust_dataset(
    ...
    if target_column not in df.columns:
        target_column = _find_column(
            df, ["classification", "class", "category", "type", "label", "target"]
        )
