...
class OpenCVVideoLoader(VideoStreamLoader):
    ...
    def open(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Opened video source: {self.source} ({width}x{height} @ {self.source_fps} FPS)"
            )
            return True
...
class VideoStreamDataset:
    ...
    def get_batch(
        self, batch_size: Optional[int] = None, timeout: float = 2.0
...
def create_video_stream(
    source: Union[str, int], config: Optional[VideoConfig] = None
) -> VideoStreamDataset:
