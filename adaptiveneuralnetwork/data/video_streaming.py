...
class VideoConfig:
    ...
    fps: Optional[float] = None  # Target FPS, None for source FPS
...
class VideoStreamLoader(ABC):
    ...
    @abstractmethod
    def read_frame(self) -> Tuple[Optional[np.ndarray], Optional[FrameInfo]]:
...
class OpenCVVideoLoader(VideoStreamLoader):
    ...
    def __init__(self, source: Union[str, int], config: VideoConfig):
...
class VideoStreamDataset:
    ...
    def __init__(self, source: Union[str, int], config: Optional[VideoConfig] = None):
        self.source = source
    ...
    def get_frame(self, timeout: float = 1.0) -> Optional[Tuple[np.ndarray, FrameInfo]]:
    ...
    def get_sequence(self, timeout: float = 1.0) -> Optional[torch.Tensor]:
    ...
    def get_batch(self, batch_size: Optional[int] = None, timeout: float = 2.0) -> Optional[torch.Tensor]:
...
def create_video_stream(source: Union[str, int], config: Optional[VideoConfig] = None) -> VideoStreamDataset:
