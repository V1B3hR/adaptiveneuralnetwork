        ...
        if "blur_kernel_size" in config and config["blur_kernel_size"] > 1:
            ...
            if len(transformed_data.shape) >= 2:
                ...
                blurred = torch.nn.functional.avg_pool2d(
                    padded.unsqueeze(0).unsqueeze(0), kernel_size, stride=1, padding=0
                ).squeeze()
...
def create_cross_domain_loaders(
    base_dataset: Dataset, batch_size: int = 64, num_domains: int = 3
) -> List[DataLoader]:
    """
    ...
    selected_configs = (
        domain_configs[:num_domains] if num_domains <= len(domain_configs) else domain_configs
    )
...
class CIFAR10Corrupted(Dataset):
    ...
    def __init__(
        ...
        if corruption_type not in self.CORRUPTION_TYPES:
            raise ValueError(
                f"Corruption type {corruption_type} not supported. "
    ...
    def _apply_corruption(self, image) -> torch.Tensor:
        ...
        if self.corruption_type == "gaussian_noise":
            ...
            if kernel_size > 1:
                ...
                for c in range(3):
                    blurred[:, c : c + 1] = conv2d(
                        img_tensor[:, c : c + 1], kernel, padding=kernel_size // 2
                    )
...
def load_cifar10_corrupted(
    ...
    train_dataset = CIFAR10Corrupted(
        root=root, train=True, corruption_type=corruption_type, severity=severity, download=download
    )
