...
def split_mnist_benchmark(
    ...
    for task_id in range(num_tasks):
        ...
        for epoch in range(epochs_per_task):
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                if batch_idx % (len(train_loader) // 4 + 1) == 0:
                    print(
                        f"Task {task_id + 1}, Epoch {epoch + 1}, Batch {batch_idx}/{len(train_loader)}, Loss: {loss_item:.6f}"
                    )
        ...
        for prev_task_id in range(task_id):
            ...
            prev_test_indices = [
                i for i, (_, label) in enumerate(test_dataset) if label in prev_classes
            ]
            ...
            prev_test_loader = DataLoader(
                prev_test_dataset, batch_size=config.batch_size, shuffle=False
            )
    ...
    total_forgetting = (
        sum(result["average_forgetting"] for result in task_results.values()) / num_tasks
    )
...
def ablation_study_sleep_phases(
    ...
    for phase_config in phase_configs:
        ...
        for epoch in range(num_epochs):
            ...
            print(
                f"Epoch {epoch + 1}: Loss={epoch_metrics['loss']:.4f}, Acc={epoch_metrics['accuracy']:.4f}"
            )
        ...
        avg_energy_efficiency = (
            sum(energy_efficiency) / len(energy_efficiency) if energy_efficiency else 0
        )
        ...
        if phase_distributions:
            ...
            for phase in ["active", "sleep", "interactive", "inspired"]:
                avg_phase_dist[f"avg_{phase}_ratio"] = sum(
                    d[phase] for d in phase_distributions
                ) / len(phase_distributions)
        else:
            avg_phase_dist = {}
...
def anxiety_restorative_analysis(
    ...
    for scenario in scenarios:
        ...
        for step in range(baseline_steps):
            ...
            scenario_results["phase_progression"].append(
                {"step": step, "phase": "baseline", **phase_stats}
            )
        ...
        for step in range(stress_duration):
            ...
            if scenario != "normal" and hasattr(model.phase_scheduler, "node_anxiety"):
                ...
                anxiety_increase = torch.full_like(
                    model.phase_scheduler.node_anxiety, loss_factor * 0.5
                )
            ...
            if hasattr(model.phase_scheduler, "node_anxiety"):
                anxiety_stats = model.phase_scheduler.get_anxiety_stats()
                scenario_results["anxiety_progression"].append(
                    {"step": baseline_steps + step, "phase": "stress", **anxiety_stats}
                )

            if hasattr(model.node_state, "energy"):
                energy_mean = model.node_state.energy.mean().item()
                scenario_results["energy_progression"].append(
                    {"step": baseline_steps + step, "phase": "stress", "mean_energy": energy_mean}
                )
            ...
            scenario_results["phase_progression"].append(
                {"step": baseline_steps + step, "phase": "stress", **phase_stats}
            )
        ...
        if scenario_results["anxiety_progression"]:
            stress_anxiety_levels = [
                m["mean_anxiety"]
                for m in scenario_results["anxiety_progression"]
                if m["phase"] == "stress"
            ]
            ...
            scenario_results["summary"] = {
                ...
                "final_recovery_anxiety": (
                    recovery_anxiety_levels[-1] if recovery_anxiety_levels else 0
                ),
                "anxiety_resilience": 1.0
                - (max(stress_anxiety_levels) if stress_anxiety_levels else 0) / 15.0,
                "recovery_effectiveness": (
                    len(
                        [
                            m
                            for m in scenario_results["recovery_metrics"]
                            if m["recovery_rate"] > 0.1
                        ]
                    )
                    / len(scenario_results["recovery_metrics"])
                    if scenario_results["recovery_metrics"]
                    else 0
                ),
            }
    ...
    results["overall_analysis"] = {
        "most_stressful_scenario": max(
            scenarios, key=lambda s: results[s].get("summary", {}).get("max_stress_anxiety", 0)
        ),
        "best_recovery_scenario": max(
            scenarios, key=lambda s: results[s].get("summary", {}).get("recovery_effectiveness", 0)
        ),
        ...
        "average_resilience": sum(
            results[s].get("summary", {}).get("anxiety_resilience", 0) for s in scenarios
        )
        / len(scenarios),
    }
