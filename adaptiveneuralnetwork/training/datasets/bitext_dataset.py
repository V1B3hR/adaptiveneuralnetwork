...
class BitextDatasetLoader:
    ...
    def _has_kaggle_credentials(self) -> bool:
        ...
        return (
            os.environ.get('KAGGLE_USERNAME') is not None and
            os.environ.get('KAGGLE_KEY') is not None
        ) or Path.home().joinpath('.kaggle', 'kaggle.json').exists()

    def _download_from_kaggle(self, dataset_name: str) -> Path | None:
        """
...
def create_synthetic_bitext_data(
    ...
    try:
        ...
        words = ["the", "quick", "brown", "fox", "jumps", "over", "lazy", "dog",
                ...
                "neural", "network", "adaptive", "system", "bitext", "dataset"]
