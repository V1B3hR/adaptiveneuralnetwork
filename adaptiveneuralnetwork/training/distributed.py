...
class DistributedTrainer:
    ...
    def _init_pytorch_distributed(self) -> None:
        ...
        self.ddp_model = DDP(self.model, device_ids=[self.config.local_rank] if torch.cuda.is_available() else None)
    ...
    def train_epoch(
        ...
        if self.is_distributed and not self.config.use_ray:
            ...
            accuracy = (metrics_tensor[1].item() * metrics_tensor[2].item()) / metrics_tensor[2].item()
    ...
    def save_checkpoint(self, filepath: str | Path, **extra_data) -> None:
...
class RayDistributedTrainer:
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("Ray not installed. Install with: pip install ray[train]") from e
    ...
    def setup_ray_training(self) -> None:
        ...
        scaling_config = ScalingConfig(
            ...
            resources_per_worker=self.distributed_config.ray_resources_per_worker or {
                "CPU": 1,
