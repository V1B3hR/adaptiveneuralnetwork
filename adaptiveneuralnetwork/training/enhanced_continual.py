...
class ProgressiveDomainShift:
    ...
    def _prepare_stage_datasets(self) -> None:
        ...
        for stage in range(self.config.num_stages):
            ...
            corruption_intensity = (
                self.config.initial_corruption +
                progress * (self.config.final_corruption - self.config.initial_corruption)
            )
            ...
            for idx in indices:
                ...
                if self.config.scenario_name == "blur_to_adversarial":
                    if stage < self.config.num_stages // 2:
                        ...
                        adv_intensity = (stage - self.config.num_stages // 2) / (self.config.num_stages // 2)
                    ...
                    if stage < self.config.num_stages // 2:
                        ...
                        rot_intensity = (stage - self.config.num_stages // 2) / (self.config.num_stages // 2)
                    ...
                    for i, corruption in enumerate(corruptions):
                        if stage >= i * (self.config.num_stages // len(corruptions)):
                            intensity = corruption_intensity * (0.5 + 0.5 * (i + 1) / len(corruptions))
                            x_corrupted = self.corruption_functions[corruption](x_corrupted, intensity)
                    ...
                    if corruption_type in self.corruption_functions:
                        x_corrupted = self.corruption_functions[corruption_type](x, corruption_intensity)
                    else:
                        x_corrupted = self._apply_noise(x, corruption_intensity)
            ...
            stage_dataset = TensorDataset(
                torch.stack(stage_data),
                torch.tensor(stage_labels)
            )
    ...
    def get_stage_dataset(self, stage: int) -> Dataset:
        ...
        if stage < 0 or stage >= len(self.stage_datasets):
            raise ValueError(f"Stage {stage} not available. Valid range: 0-{len(self.stage_datasets)-1}")
    ...
    def get_stage_info(self, stage: int | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "dataset_size": len(self.stage_datasets[stage]) if stage < len(self.stage_datasets) else 0,
            "progress": progress,
        }
...
class ContinualLearningTrainer:
    ...
    def train_stage(
        ...
        for epoch in range(epochs):
            ...
            print(f"Stage {stage_info['stage']}, Epoch {epoch}: "
