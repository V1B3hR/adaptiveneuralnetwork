...
class GenericV3Backend(BaseHardwareBackend):
    ...
    def compile_network(self, model: nn.Module) -> Dict[str, Any]:
        ...
        logger.info(
            f"V3 compilation complete. {total_neurons} neurons, "
    ...
    def _get_neuron_model_config(self, module: nn.Module) -> Dict[str, Any]:
        ...
        if "MultiCompartment" in module_type:
            ...
            config.update({"base_type": "adaptive_lif", "adaptive_threshold": True})
        elif "Bursting" in module_type:
            config.update({"base_type": "bursting", "burst_capable": True})
        elif "Stochastic" in module_type:
            config.update({"base_type": "stochastic_lif", "stochastic": True})
    ...
    def _extract_hierarchical_connections(
        self, connections: nn.ModuleList, connection_type: str
    ) -> List[Dict[str, Any]]:
    ...
    def _has_plasticity(self, connection: nn.Module) -> bool:
        ...
        return any(
            "plasticity" in attr.lower() or "stdp" in attr.lower() for attr in dir(connection)
        )
    ...
    def _is_synaptic_connection(self, module: nn.Module) -> bool:
        ...
        return (
            any(stype in type(module).__name__ for stype in synapse_types)
            or hasattr(module, "synaptic_weights")
    ...
    def _simulate_v3_execution_metrics(
