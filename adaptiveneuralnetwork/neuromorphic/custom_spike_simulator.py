...
class CustomSpikeSimulator(BaseHardwareBackend):
    ...
    def _extract_network_structure(self, model: nn.Module) -> Dict[str, Any]:
        ...
        structure = {"layers": [], "connections": [], "neuron_counts": [], "total_neurons": 0}
        ...
        for layer in network_config["layers"]:
            ...
            for i in range(layer["output_size"]):
                neuron = self._create_single_neuron(
                    neuron_id, layer["neuron_model"], layer["parameters"]
                )
    ...
    def _create_single_neuron(
        self, neuron_id: int, model: NeuronModel, params: NeuronParameters
    ) -> Dict[str, Any]:
    ...
    def _create_multicompartment_neuron(
        self, neuron_id: int, params: NeuronParameters
    ) -> Dict[str, Any]:
    ...
    def execute(
        ...
        avg_spike_rate = (
            total_spikes / (simulation_duration / 1000.0) if simulation_duration > 0 else 0
        )
        ...
        metrics = HardwareMetrics(
            ...
            synaptic_operations_per_second=(
                total_synaptic_ops / (simulation_duration / 1000.0)
    ...
    def _update_synapses(self) -> int:
        ...
        for synapse_id, synapse in self.synapses.items():
            ...
            if pre_neuron and post_neuron:
                ...
                if (self.simulation_time - pre_neuron["last_spike_time"]) < (
                    synapse["parameters"].delay * self.time_step
                ):
    ...
    def _find_neuron_by_id(self, neuron_id: int) -> Optional[Dict[str, Any]]:
    ...
    def _collect_output(self, batch_size: int) -> torch.Tensor:
        ...
        for i, (neuron_id, neuron) in enumerate(last_layer_neurons.items()):
            ...
            recent_spikes = sum(
                1 for t in neuron["spike_history"] if self.simulation_time - t < recent_window
            )
    ...
    def get_simulation_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_firing_rate": (
                np.mean(
                    [
                        len(spikes) / (self.simulation_time / 1000.0)
                        for spikes in self.spike_times.values()
                    ]
                )
                if self.spike_times
                else 0.0
