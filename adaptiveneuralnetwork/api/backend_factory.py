...
from ..core.jax_backend import is_jax_available, JAXAdaptiveModel, JAXNodeConfig, convert_pytorch_to_jax_config
    ...
    def create_model(config: AdaptiveConfig) -> Union[AdaptiveModel, 'JAXAdaptiveModel', NeuromorphicAdaptiveModel]:
        ...
        if backend == "pytorch":
            ...
            raise ValueError(f"Unsupported backend: {backend}. "
                           f"Supported backends: pytorch, jax, neuromorphic")
...
def get_backend_info() -> dict[str, dict]:
    ...
    info = {
        "pytorch": {
            ...
            "features": ["Dynamic computation graphs", "CUDA acceleration", "Automatic differentiation"],
            "dependencies": ["torch", "torchvision"]
        },
        "jax": {
            "available": is_jax_available(),
            "description": "JAX-based backend for functional programming and advanced acceleration",
            "features": ["JIT compilation", "Functional programming", "Auto-vectorization", "TPU support"],
            "dependencies": ["jax", "jaxlib", "flax", "optax"]
        },
        "neuromorphic": {
            "available": True,
            "description": "Neuromorphic hardware compatibility layer with spike-based processing",
            "features": ["Spike-based computation", "Event-driven processing", "Hardware simulation"],
