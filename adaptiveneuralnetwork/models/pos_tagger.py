...
def create_pos_tagger(
    vocab_size: int, num_tags: int, model_type: str = "bilstm", **kwargs
) -> POSTagger:
    """
    ...
    config = POSTaggerConfig(
        vocab_size=vocab_size, num_tags=num_tags, model_type=model_type, **kwargs
    )
...
def compute_token_accuracy(
    predictions: torch.Tensor, labels: torch.Tensor, mask: torch.Tensor
) -> float:
    """
