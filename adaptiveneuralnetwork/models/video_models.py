...
class ConvLSTMCell(nn.Module):
    ...
    def init_hidden(
        self, batch_size: int, image_size: Tuple[int, int], device: torch.device
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Initialize hidden and cell states."""
        height, width = image_size
        h = torch.zeros(
            batch_size, self.hidden_dim, height, width, dtype=torch.float, device=device
        )
        c = torch.zeros(
            batch_size, self.hidden_dim, height, width, dtype=torch.float, device=device
        )
...
class Conv3D(nn.Module):
    ...
    def __init__(self, config: VideoModelConfig):
        ...
        self.conv3d_layers = nn.Sequential(
            ...
            nn.Conv3d(
                256, config.hidden_dim, kernel_size=(3, 3, 3), stride=(2, 2, 2), padding=(1, 1, 1)
            ),
...
class HybridVideoModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, fusion_mode: str = "weighted") -> torch.Tensor:
        ...
        if fusion_mode == "weighted":
            ...
            output = (
                weights[0] * convlstm_out + weights[1] * conv3d_out + weights[2] * transformer_out
            )
...
def create_video_model(
    model_type: str, config: Optional[VideoModelConfig] = None, **kwargs
) -> nn.Module:
