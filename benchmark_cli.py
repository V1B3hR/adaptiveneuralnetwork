...
Examples:
  ...
  python benchmark_cli.py --run-robustness --no-stress-tests
    ...
    # Main actions
    parser.add_argument(
        "--run-benchmark", action="store_true", help="Run comprehensive intelligence benchmark"
    )
    parser.add_argument(
        "--run-robustness", action="store_true", help="Run comprehensive robustness validation"
    )
    parser.add_argument(
        "--run-adversarial", action="store_true", help="Run adversarial signal benchmark tests"
    )
    parser.add_argument(
        "--run-combined",
        action="store_true",
    ...
    parser.add_argument(
        "--compare", metavar="BASELINE_FILE", help="Compare with baseline model from JSON file"
    )

    # Options
    parser.add_argument("--output", "-o", metavar="FILE", help="Output file for reports")
    parser.add_argument(
        "--save-results", metavar="FILE", help="Save benchmark results to JSON file"
    )
    parser.add_argument(
        "--no-comparisons", action="store_true", help="Skip generating comparison baselines"
    )
    parser.add_argument(
        "--no-stress-tests",
        action="store_true",
    ...
    parser.add_argument(
        "--ethics-only", action="store_true", help="Run only ethics compliance checks"
    )
    ...
    if not (
        args.run_benchmark
        or args.run_robustness
        or args.run_adversarial
        or args.run_combined
        or args.generate_report
        or args.generate_comprehensive_docs
        or args.compare
        or args.ethics_only
    ):
        parser.error(
            "Must specify an action: --run-benchmark, --run-robustness, --run-adversarial, --run-combined, --generate-report, --generate-comprehensive-docs, --compare, or --ethics-only"
        )
...
def run_combined_action(benchmark, args):
    ...
    results = benchmark.run_comprehensive_benchmark(
        include_comparisons=include_comparisons, include_robustness=True
    )
    ...
    if "combined_intelligence_robustness_score" in results:
        print(
            f"Combined Intelligence + Robustness Score: {results['combined_intelligence_robustness_score']:.1f}/100"
        )
...
def run_comprehensive_docs_action(args):
    ...
    try:
        ...
        with open(args.save_results) as f:
            results = json.load(f)
