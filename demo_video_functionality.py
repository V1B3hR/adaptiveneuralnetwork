...
def create_synthetic_video_data(batch_size=4, sequence_length=16, height=224, width=224):
    ...
    print(f"Creating synthetic video data: {batch_size} sequences of {sequence_length} frames ({height}x{width})")
...
def compare_models():
    ...
    if results:
        ...
        for result in results:
            params_str = f"{result['num_parameters']/1000:.0f}K" if result['num_parameters'] < 1000000 else f"{result['num_parameters']/1000000:.1f}M"
            print(f"{result['model_name']:<15} {params_str:<10} {result['mean_latency_ms']:<12.1f} {result['throughput_per_sec']:<12.2f}")
        ...
        print(f"\nHighlights:")
        print(f"  Fastest model: {fastest_model['model_name']} ({fastest_model['mean_latency_ms']:.1f}ms)")
        print(f"  Most efficient: {most_efficient['model_name']} ({most_efficient['num_parameters']/1000:.0f}K params)")
